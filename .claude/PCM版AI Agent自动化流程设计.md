...
计时独立保存在 v2 `runs/<run-id>/timings.json`，时间戳统一北京时间 `+08:00`。可观测中断记录时刻，不可捕获退出保持未知；缺口仅以步骤级提示说明，不重复业务适用性与复用布尔。旧 v1 只读查看不修改，新进程首次写入时原样归档后升级，不把旧命令耗时冒充 Agent 时间。计时不可用只警告，不改变业务三态、prompt、负责人裁决、恢复和退出码。字段定义见 [计时记录与字段说明](../pcm-demo/docs/timing.md)，不新增业务节点或外部监控服务。
