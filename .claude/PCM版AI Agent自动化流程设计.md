...
- 输出：`project-intake` 生成的项目需求说明和产品功能说明、Claude Agent session ID、决策模型对话历史及步骤结果。
- 完成条件：目标 Skill 已加载并显式调用；Agent 正常完成；Skill 规定的产品定义产物真实存在且可读；Claude session 和决策历史均已持久化；没有外部资源阻塞。
