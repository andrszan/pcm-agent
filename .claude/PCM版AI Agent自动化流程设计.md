...
Demo 的单步和阶段一完整编排共用重试循环外层计时，不改变业务步骤、三态结果、模型 prompt、负责人裁决、完成条件或恢复规则。累计运行耗时包含实际执行、验证、多轮沟通、自动重试及其等待，排除进程退出后的停机间隔；保留 UTC 起止时间，记录完整时另显示首次开始到首次成功的总历时。第 0～12 步按项目、第 13～18 步按需求 ID 和步骤编号区分，失败与阻塞耗时保留，恢复后累计，成功复用不覆盖原始完成耗时。

计时独立保存在 `runs/<run-id>/timings.json`，只用于终端显示与汇总，不参与业务状态判断。旧历史缺失和未闭合执行如实标记，不推算未知耗时；计时文件不可用只警告，不改变业务退出码。实现细节以 Demo 活动 TRD 和 README 为准，不引入监控平台或新的流程节点。
