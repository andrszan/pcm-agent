...
- `assets/layout-source-snapshots/`：带实际预览的部分框架源码快照，保留上游依赖，不保证独立运行；技术兼容且适用的代码可以复用。
...
源码快照的每个 provider 保留 README；每个语义目录保留 README、`source/` 和实际存在的预览。记录可核验的技术版本、依赖、快照完整性、局限和预览信息，不承诺部分快照独立运行。
