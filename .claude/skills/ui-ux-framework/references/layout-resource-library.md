...
本文件是 `ui-ux-framework` 布局资源的索引与维护规则。当前随附 `assets/layout-source-snapshots/` 中的部分框架源码快照和桌面预览；它们带有上游依赖，不保证独立运行，也不代表项目默认布局或技术选型。
...
4. 允许复用技术兼容且适合目标任务的布局结构、组件组合和响应式代码。优先接入项目已有组件、token、路由、权限与状态；尺寸和断点按真实内容和目标视口核验，可保留合适值，不要求为了原创重新实现。
5. 不照搬不适用的导航、品牌、fixture、占位内容、假链接或 demo logic。
...
这些 Block-derived installed-output snapshots 带有 React、Tailwind CSS、shadcn/ui 及相关生态依赖痕迹，缺少完整宿主配置，不能独立编译或运行。它们可用于理解结构与交互，并在技术兼容时复用适用代码。
...
四套快照均未附 CLI 版本、upstream commit、精确安装命令和导入前修改记录，只有实际随附的桌面预览，没有移动端预览；预览不作为目标项目需求或视觉验收基线。具体依赖、快照完整性和预览信息见 provider 与对应资源 README。
...
- Provider README 说明共同依赖、快照映射、完整性和预览信息；子 README 说明适用任务、结构、关键局限、源码与预览。
- 保持源码相对结构，整理后的语义目录不冒充官方原始目录。新收录时记录可核验的版本或 commit；没有版本信息时不补造。
- 明确完整模板与部分快照的区别。部分快照不要求独立构建，不伪造未提供的移动端预览、交互验证或技术版本记录。
- 变更后更新本索引，核对相对链接和源码／预览对应关系；只在行为边界变化时更新 Skill 和 eval。

## 4. 预览用途

现有源码附带的预览用于理解区域比例、信息层级和审美方向，不要求另建图片库或为每种布局补充资产。优先利用现有参考，由 Agent 按项目选择、组合和完善；具体页面美化留给实际设计与开发。

如调用方提供额外图片，只作为视觉参考；生成图不能冒充真实产品或交互验证。
