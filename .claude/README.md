...
- “开发验收数据基线”是目标项目受版本控制维护的一组产品内账号与角色、必要参考数据、代表性业务对象与状态、适用对象资源、恢复方式、验证方式和交付说明，使接手者能在项目专用开发或验收环境中从正常入口自主体验主要已交付功能。基线定义、恢复/核验实现和交付说明受版本控制，环境中的实际数据按这些内容建立和恢复。
- 它不同于 `project-readiness` 负责的外部服务、外部账号、运行凭据、私有数据和设备等资源准备，也不同于 `media-assets` 自带的测试 fixture；任意测试临时数据、生产数据、真实个人数据，以及 PCM 或目标产品访问其它系统或资源的凭据都不属于开发验收数据基线。
- 恢复方式依据项目事实选择，可以采用非破坏 initialize、独立 verify、重建专用环境后 Seed，或仅对管理范围明确的专用环境执行严格受控的 destructive reset。不得强制统一命令、技术栈、Manifest 或原地 reset；重复执行应不产生重复基线数据、不覆盖管理范围外内容，破坏性方式必须明确适用环境、范围、影响和安全前置条件。
- 项目 README 或既有受跟踪交付文档必须说明适用环境、前置服务、恢复与核验入口、管理范围、数据影响、账号文档位置、代表性业务对象及其资源范围和成功判断。账号另行成文时必须可从上述入口发现；基线内容变化时同步文档。
- 最终保留在基线中且交付后仍可登录的产品内开发账号及密码，都是受版本控制的产品交付信息，包括普通用户、业务角色、产品管理员和产品超级管理员。账号应项目专用、不冒用真实个人身份、在恢复后真实存在且可从正常入口登录；不得称为演示账号、体验账号或假账号，也不得附加演示租户、隔离业务数据或生产禁用 Seed 等部署阶段限制。账号文档至少记录适用环境、登录入口、角色、账号、密码、恢复方式和必要已有数据说明；完成前逐个核验存在性、登录、角色和数据可见性。临时测试后删除或事务回滚、未保留在最终交付开发状态中的短期账号和数据不进入基线。
- 凭据是否公开按认证目标判断：用于登录目标产品的基线账号公开；PCM 或目标产品用于访问其它系统或资源的数据库、对象存储、SMTP、OAuth client secret、API key、私钥、令牌、Git、云、服务器、基础设施账号、PCM 工具和媒体 Provider 凭据及真实个人认证信息仍属秘密。`docs/ignore/` 可继续保存真正私密的开发资源资料，但不承担目标产品账号或开发验收数据基线交接。
...
| Skill | 职责 |
|---|---|
| `project-intake` | 通过对话收敛最终产品范围、核心任务、用户语言、复杂度取舍、初始可体验内容或空态首个成功路径，以及需要尽早验证的高风险假设 |
...
| `product-experience-audit` | 对当前完整集成产品执行跨需求、跨模块、跨页面的全项目级体验审计，优先消费并核验开发验收数据基线，输出经核验候选、重复项和覆盖缺口，不修改正式 Backlog 或自行修复基线 |
...
| `requirement-breakdown` | 完整覆盖最终产品范围，拆成有明确结果、依赖和验收方向的 Backlog，将开发验收数据基线落实为全局完成原则和 owning BR 增量，并建议首条验证切片 |
| `trd-design` | 为一个内聚需求设计产品行为、体验复杂度、关键内容意图、技术实现及适用的开发验收数据基线增量，形成或更新活动 TRD |
| `dev-workflow` | 实现功能、Bug 或重构，维护 canonical 开发验收数据基线，以自动化、真实运行、浏览器、实际渲染和独立审查证明功能与体验结果 |
...
`media-assets` 的 `providers/` 维护显式准入的 Provider catalog 和适配说明，不扫描目录或自动安装来源；`assets/fixtures/` 是该工具受跟踪测试输入的维护入口，不承载目标产品资产、对象存储数据、开发验收数据基线或工作区私有凭据。
...
- 开发 `.env` 可以在项目范围内安全使用；开发验收数据基线的定义、恢复/核验实现和交付说明受版本控制，用于登录目标产品的基线账号及密码进入受跟踪账号文档，PCM 或目标产品用于访问其它系统或资源的凭据及其它秘密必须保持 Git 忽略并在输出中脱敏。
...
3. 根据目标项目的版本文件、锁文件和说明准备运行时、包管理器、依赖、数据库、服务、迁移、初始化数据与浏览器验证条件；业务开发开始后按产品范围逐步建立开发验收数据基线，并在项目 README 或既有受跟踪交付文档中维护适用环境、前置服务、恢复/核验、管理范围、数据影响、账号文档、对象资源范围和成功判断。
4. 补齐被 Git 忽略的开发 `.env` 或等价本地配置，不复制、提交或输出原项目用于访问外部系统或资源的真实凭据及真实个人认证信息；目标产品最终保留的基线账号及密码按新项目事实写入其受跟踪账号文档。
5. 从当前目标需要的独立能力开始；空白项目通常先收敛产品定义，再根据实际不确定性选择技术设计、项目准备核验、工程项目化或其它适用步骤。
6. 使用目标项目自身的测试、构建、服务、数据和浏览器环境验证，不假设所有项目采用同一技术栈或工具组合。
7. 进入产品体验审计时，优先按受跟踪交付说明恢复并核验开发验收数据基线，再使用其中项目专用且真实可登录的产品账号、代表性业务对象与状态和可复位数据执行主要任务；基线缺失或失效时保留覆盖缺口，不由审计能力自行修改。缺少固定浏览器工具不会触发自动安装，但缺少必要动态证据时必须保留覆盖缺口。
