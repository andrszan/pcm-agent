...
`conversations/*.json` 保存负责人对话。Agent 回复保留本次 SDK 调用内主 Agent 的可见文字：有过程内容时明确分为“过程说明”和“最终回复”，最终回复决定当前状态，过程只补充依据；不包含 thinking、工具结果、子 Agent 原始日志或此前整段 session。只有最终回复时保持原纯文本格式。
