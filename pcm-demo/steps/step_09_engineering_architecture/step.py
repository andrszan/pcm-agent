...
ARCHITECTURE_REPAIR_PROMPT = (
    ...
    "不适用必须基于事实，不得虚构。仍只维护根仓这一份文档，不得创建 frontend/backend 本地架构文档。"
...
ENGINEERING_ARCHITECTURE_DECISION_RULES = """- completed：固定工程架构设计文档已生成，且仍为根仓唯一的 `docs/design/工程架构设计.md`，没有创建 frontend/backend 本地架构文档；每个适用且含相关业务代码的交付单元均分别有有限 Current/Target 地图，使用 [当前]/[目标]/[按需]/[迁移] 标注，明确目录/模块职责、公开/私有边界、允许/禁止依赖，并至少有一个代表性文件放置演练，说明最小迁移和可观察演进，根仓或另一单元概述不能替代。frontend 存在时，已按实际工程覆盖应用装配/路由页面、业务功能、远程/局部/跨页面状态、传输到展示模型映射和共享 UI 准入；backend 存在时，已按实际工程覆盖入口/契约、编排/规则、持久化/外部适配及事务/授权/错误/副作用/重试/恢复。双端均含业务代码时，frontend 和 backend 各至少有一个演练；不适用已有事实依据，未虚构。架构决策矩阵有决策证据，当前下游所需的高影响架构决定已收敛，且没有越界实现业务功能。不得固定框架、FSD、DDD、Clean、目录模板或行数阈值；千行仅触发职责调查。
- continue：上述逐交付单元工程归属合同、工程事实核验或下游所需高影响架构决定仍可依据当前事实补全。
...
文档应把权威输入和实际工程收敛为根仓唯一单份、逐交付单元闭合的工程归属合同，而非通用技术清单：每个适用且含相关业务代码的交付单元均须分别给出有限 Current/Target 地图，使用 [当前]/[目标]/[按需]/[迁移] 区分已证实事实、已收敛目标、仅在有证据和消费者时才需要的能力及必要迁移；明确目录/模块职责、公开/私有边界、允许/禁止依赖，并至少有一个代表性文件放置演练，说明最小迁移和可观察演进，根仓或另一单元概述不得替代，不得虚构 Current。frontend 存在时按实际工程覆盖应用装配/路由页面、业务功能、远程/局部/跨页面状态、传输到展示模型映射和共享 UI 准入；backend 存在时按实际工程覆盖入口/契约、编排/规则、持久化/外部适配及事务/授权/错误/副作用/重试/恢复。双端均含业务代码时，两端各至少有一个演练；不适用必须基于事实。用架构决策矩阵记录决定、证据、影响和状态，收敛当前下游所需的高影响架构决定。仍只维护根仓 `docs/design/工程架构设计.md`，不得创建 frontend/backend 本地架构文档。不得固定框架、FSD、DDD、Clean、目录模板或行数阈值；千行仅触发职责调查，不是强制拆分。不得预建 `domain`、`application`、`infrastructure`、`shared`、服务、队列、接口或无消费者结构，不得无边界全仓扫描。配置、运行、数据、测试、安全等主题仅按权威输入或实际工程证据展开，不机械罗列。
