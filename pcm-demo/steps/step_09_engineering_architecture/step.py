...
ARCHITECTURE_REPAIR_PROMPT = (
    ...
    "必须按当前工程架构合同补齐有限 Current/Target 地图、目录/模块职责、公开边界、"
    "2-5 个代表性文件放置演练和必要迁移，并使用 [当前]/[目标]/[按需]/[迁移] 标注；"
    "不得只写空壳、不得虚构 Current、不得创建目标目录或预留抽象，然后报告结果。"
)
...
ENGINEERING_ARCHITECTURE_DECISION_RULES = """- completed：固定工程架构设计文档已生成；每个权威工程都有有限 Current/Target 地图，使用 [当前]/[目标]/[按需]/[迁移] 标注；架构决策矩阵有决策证据，明确目录/模块职责、公开边界、语义所有权、稳定公开能力、私有禁区、允许/禁止依赖和共享准入，包含 2-5 个代表性文件放置演练、最小迁移和可观察演进；当前下游所需的高影响架构决定已收敛，且没有越界实现业务功能。
- continue：上述工程归属合同、工程事实核验或下游所需高影响架构决定仍可依据当前事实补全。
...
文档应把权威输入和实际工程收敛为工程归属合同，而非通用技术清单：为每个权威工程给出有限 Current/Target 地图，使用 [当前]/[目标]/[按需]/[迁移] 区分已证实事实、已收敛目标、仅在有证据和消费者时才需要的能力及必要迁移；不得虚构 Current。用架构决策矩阵记录决定、证据、影响和状态，明确目录/模块职责、公开边界、语义所有权、稳定公开能力、私有禁区、允许/禁止依赖和共享准入；用 2-5 个代表性文件放置演练说明具体改动应落到哪里，并给出最小迁移和可观察演进。MVC、分层、六边形、DDD 不是四选一；不得预建 `domain`、`application`、`infrastructure`、`shared`、服务、队列、接口或无消费者结构，不得无边界全仓扫描。配置、运行、数据、测试、安全等主题仅按权威输入或实际工程证据展开，不机械罗列。当前下游所需的高影响架构决定应已收敛。
