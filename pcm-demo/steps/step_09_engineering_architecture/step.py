...
ARCHITECTURE_REPAIR_PROMPT = "检测到 `docs/design/工程架构设计.md` 缺失或为空。请按原授权仅创建或补全该固定产物，不修改其它文件或执行 Git 写操作。"
...
ENGINEERING_ARCHITECTURE_DECISION_RULES = """- completed：固定工程架构设计文档已生成。
...
请依据以下权威资料和实际工程创建或更新唯一固定产物 `docs/design/工程架构设计.md`。
...
只允许修改该固定产物，不得创建各工程本地架构文档、实现业务功能、修改代码配置或项目规则、执行 Git 写操作或处理秘密。"""
