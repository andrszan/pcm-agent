...
PROJECT_READINESS_DECISION_RULES = """completed 表示当前开发的资源准备基线已经建立：完成编码、开发环境联调和开发环境真实验收所需的外部服务、账号、凭据、授权素材、私有数据或专用设备均已识别；每项开发必需外部资源已真实可用，或有事实依据地判为当前开发不需要。每项 ready 外部运行资源的最终项目凭据与资源绑定都已持久化到所属仓库受保护的实际 `.env` 或等价配置，公开键合同已同步，含秘密文件权限安全（POSIX 通常为 0600），并使用最终写入配置的运行凭据完成最小行为和隔离验证。不存在以下两类阻塞：开发必需外部资源在候选池中缺失、当前环境无法安全生成且无兼容替代；已匹配资源真实不可用、凭据无效、权限不足、隔离不合格，或缺少开发所需接口能力、可用配额、回调/白名单及其它既定能力。依赖安装、业务实现、migration、Seed、产品内账号、完整联调和浏览器验收属于后续开发工作；生产部署环境、正式域名、DNS/TLS、生产凭据、监控、备份、容量和发布安全属于开发范围外未来事项；这些 missing 或 pending 均不得阻塞 completed。管理凭据、资源说明、Mock、截图、清单文字或 Agent 自述不能单独证明 ready，且没有执行 Git 写操作。
continue 表示仍有可使用当前资料和工具安全完成的开发资源匹配、创建、项目凭据持久化、公开配置键合同、文件权限、最小行为/隔离验证或证据补全工作。产品规则或技术细节尚待设计、业务尚未实现、生产发布条件缺失时，不返回 continue 试图在本步骤解决，只需记录为非阻塞后续事项。
blocked 只允许两种情况：一、已确认当前开发必须依赖的外部资源在调用方候选池中不存在、当前环境无法安全生成且没有兼容替代；二、已匹配的开发资源经过真实验证不可用、凭据无效、权限不足、隔离不符合开发要求，或缺少开发所需接口能力、可用配额、回调注册、白名单、沙箱范围及其它既定合同能力。required_inputs 必须精确指出缺少或不可用的开发资源、能力和复验条件；不得因生产发布条件、业务实现、产品规则待定或未来最终验收返回 blocked。"""
...
该清单是任意格式的动态候选池，不预设其中的资源类型、字段、服务名称或环境变量。请从产品范围和当前工程事实推导完成编码、开发环境联调和开发环境真实验收所需、且必须由调用方提供的外部服务、账号、凭据、授权素材、私有数据或专用设备，再按既定能力、兼容、安全和隔离边界匹配候选；改变产品或技术合同的替代不能静默采用。

在调用方授权、项目配置合同和安全边界允许时，实际准备项目专用且环境隔离的开发/测试资源与最小权限运行凭据。对每项当前开发需要且准备完成的外部运行资源，必须把最终项目运行凭据和资源绑定持久化到所属仓库被 Git 忽略的实际 `.env` 或等价受保护配置，确保后续开发无需重新读取共享资源清单即可使用；即使消费代码稍后实现，只要配置键、归属和用途能够由已确认技术合同、资源合同或工程惯例唯一确定，也应先建立最小配置键合同。同步不含秘密的 `.env.example` 或既有公开配置示例，并参考资源资料中的公开说明补充必要注意事项和用法；包含秘密的实际配置在适用平台上必须仅当前用户可读写（POSIX 通常为 `0600`）。必须使用最终写入项目配置的运行凭据验证身份、权限、隔离和所需最小行为，不能用管理凭据、资源说明、Mock、截图或文档自述替代。

只允许以下两类阻塞：开发必需外部资源在候选池中不存在、当前环境无法安全生成且无兼容替代；或已匹配资源真实验证不可用、凭据无效、权限不足、隔离不合格，或缺少开发所需接口能力、可用配额、回调/白名单、沙箱范围及其它既定能力。依赖安装、业务代码、migration、Seed、项目内测试账号、完整联调和浏览器验收由后续开发完成，不属于资源缺失。生产部署、正式域名、DNS/TLS、生产凭据、监控、备份恢复、容量和发布安全明确属于当前自动化开发范围外，缺失时只能记录为非阻塞未来事项。产品规则、隐私保留和其它设计决定交给后续产品/技术设计，不得因此阻塞。
...
def completion_repair_prompt() -> str:
    return (
        ...
        "只有开发必需外部资源缺失、当前环境无法安全生成且无替代，或已匹配资源不可用、权限/隔离/必要能力/配额/回调/白名单不符合开发合同时才能报告阻塞。生产发布和后续业务实现事项必须记录为非阻塞。"
    )
