...
PROJECT_READINESS_MAX_TURNS = 9999
...
continue：仍有可用当前资料和工具安全完成的资源准备、配置修正、验证或清单清理工作。只补齐缺失、失效或不合格绑定；删除清单中的未采用候选和范围外事项，不换个标签保留，也不开展范围外工作。"""
...
请依据以下权威输入建立开发资源准备基线，并将脱敏结果写入唯一固定产物 `docs/requirements/项目准备清单.md`。
...
不得泄露秘密、改变 Git 边界或执行 Git 写操作。"""


def completion_repair_prompt() -> str:
    return "检测到 `docs/requirements/项目准备清单.md` 缺失或为空。请按原授权完成必要资源准备与真实验证，并写入该固定产物；不要只补空泛说明。"
