...
REQUIREMENT_BREAKDOWN_DECISION_RULES = """- completed：固定 Backlog 文档已生成，基于权威产品定义、项目准备清单、总体技术方案、工程架构设计、产品级体验框架（如适用）和实际工程，形成可独立交付、可验证、顺序合理的正式需求条目，说明范围、目标、验收要点、依赖和风险；已确认 Target 只有同时要求演进既有产品表面、迁移本身形成独立可观察的用户结果、且其它需求开始前确实必须完成时，才创建迁移 BR 和严格依赖，不满足时写为相关 BR 的体验约束；默认 Target 只作为体验约束并保留依据和重议条件，不创建迁移 BR 或严格依赖；偏离默认 Target 或改变跨需求体验骨架时列为具体待确认；开发验收数据基线已同时落实为全局完成原则和 owning BR 增量，后续影响角色、对象、关键状态或对象文件的 BR 负责同步基线增量与交付文档；不机械创建独立 Seed BR，不把 DB、S3/对象存储、fixture、Seed 或恢复命令等实现载体写成 depends_on，也不强制 destructive reset，恢复方式由项目在幂等重跑或补齐等非破坏方式、定向重置、可重建开发环境中按实际支持选择；框架文档、页面、组件、CSS、目录、工程依赖、外部条件及其它非正式 BR ID 均不得写成严格依赖；depends_on 只指向开始前必须完成的正式 BR ID；不把需求开发状态写入 Backlog，不越界实施需求。
...
Backlog 必须形成可独立交付、可验证、顺序合理的正式需求条目，说明每项需求的范围、目标、验收要点、依赖和风险，保持与产品、设计和工程事实一致。处理跨需求体验决定时，只有已确认 Target 同时要求演进既有产品表面、迁移形成独立可观察的用户结果且其它需求开始前确实必须完成，才创建迁移 BR 和严格依赖；否则写为相关 BR 的体验约束。默认 Target 只作为体验约束并保留依据和重议条件，不创建迁移 BR 或严格依赖；偏离默认 Target 或改变跨需求体验骨架时列为待确认。

将开发验收数据基线同时落实为全局完成原则和 owning BR 增量；后续任何影响角色、对象、关键状态或对象文件的 BR，负责同步基线增量与交付文档。不得机械创建独立 Seed BR，不得把 DB、S3/对象存储、fixture、Seed 或恢复命令等实现载体写成 depends_on；depends_on 仍只指向开始前必须完成的正式 BR ID。不得强制 destructive reset；恢复方式由项目在幂等重跑或补齐等非破坏方式、定向重置、可重建开发环境中按实际支持选择。

不得在 Backlog 中写入“待开发”“开发中”“已完成”“阻塞”等需求开发状态；业务对象或业务流程自身的状态可作为需求内容。不得猜测缺失事实或将未确认事项当作已确认决定。
