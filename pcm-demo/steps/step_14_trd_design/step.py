...
判断活动 TRD 是否足以指导实现，不以还能补充更多细节、风险或验证方法作为继续理由。普通内部实现细节不必在设计阶段冻结；没有已确认容量、延迟或运行规模依据时，不为结束设计自行生成硬指标或完整性能工程。Agent 提出显著增加实现或验证成本的机制时，先核对它保护的当前用户结果、数据、安全或兼容性要求，并比较能保留这些结果的更简单方案。但不得以精简为由缩减正式功能或已确认底线；需要补充时只处理具体问题和受影响内容，不重新全面审查已经收敛的部分。"""
...
唯一输出：`{trd_path}`
正式 Backlog 及项目资料、实际工程、代码、测试、接口、数据、权限、配置和运行约定请按需读取。

只允许创建或更新该活动 TRD；不得实现业务功能、修改其它文件、执行 Git 写操作、切换或创建分支、处理或展示秘密。"""
