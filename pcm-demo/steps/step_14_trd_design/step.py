...
TRD_DESIGN_DECISION_RULES = """- completed：Agent 已将当前正式需求的活动 TRD 写入唯一指定路径，范围、关键行为、技术方案、验证场景和需求级体验设计已经收敛，且没有阻碍实现的未决事项。有可核验且适用的工程架构资料时，TRD 已按每个受影响交付单元记录稳定业务 owner、目录/包/模块边界、公开/私有边界、允许/禁止依赖和预期改动归属；边界内部文件粒度可调整，但跨所有者合并、边界打平、公开出口绕过、私有路径引用或用巨型入口/页面、通用收纳目录、同名平铺文件替代已确认边界，均已作为架构 delta 说明影响、理由和最小迁移，没有静默降级。
