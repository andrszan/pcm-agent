...
TRD_DESIGN_DECISION_RULES = """- completed：Agent 已将当前正式需求的活动 TRD 写入唯一指定路径，范围、关键行为、技术方案、验证场景和需求级体验设计已经收敛，且没有阻碍实现的未决事项。有可核验且适用的工程架构资料时，TRD 已按每个受影响交付单元记录稳定业务 owner、目录/包/模块边界、公开/私有边界、允许/禁止依赖和预期改动归属；边界内部文件粒度可调整，但跨所有者合并、边界打平、公开出口绕过、私有路径引用或用巨型入口/页面、通用收纳目录、同名平铺文件替代已确认边界，均已作为架构 delta 说明影响、理由和最小迁移，没有静默降级。当前需求适用开发验收数据基线时，TRD 已设计本需求负责的基线增量与 ownership，覆盖稳定身份、幂等建立或更新、漂移处理、相对时间、项目实际支持的恢复方式和管理范围；恢复可按实际选择幂等重跑或补齐等非破坏方式、定向重置、可重建开发环境，不强制 destructive reset；同时涉及 DB 与 S3/对象存储时已定义跨存储一致性，并明确交付文档与真实验证。当前需求不适用时不机械增加基线章节或虚构设计。
- continue：当前环境仍可继续补全活动 TRD、保真承接适用工程归属合同、设计适用的开发验收数据基线增量、处理架构 delta 或收敛会影响实现的产品与技术决定时，给出明确的下一步指令。
...
项目资料、正式 Backlog、实际代码、测试、接口、数据、权限、配置和运行约定请按需自行读取。请收敛当前需求的范围、关键行为、状态与恢复、技术职责、数据与接口语义、安全边界、实现约束、需求级体验设计和可执行验证场景。对本需求真正适用的体验决定，按需从任意来源发现已确认的 Target 或有依据的默认 Target，并在 TRD 记录来源、适用范围、经核验的 Current、Target，以及本需求遵循或改变；默认 Target 须写明依据与重议条件，偏离须说明理由，改变跨需求骨架须由负责人决定。没有适用决定时不得虚构或阻塞；不要求固定框架文档、固定资料来源或技术栈。有可核验且适用的工程架构资料时须按需读取，并按每个受影响交付单元在 TRD 记录稳定业务 owner、目录/包/模块边界、公开/私有边界、允许/禁止依赖和预期改动归属；稳定边界内部文件名、数量和等价拆分可调整，跨所有者合并、边界打平、公开出口绕过、私有路径引用或用巨型入口/页面、通用收纳目录、同名平铺文件替代已确认边界，须作为架构 delta 记录影响、理由和最小迁移，不得静默降级。

仅当当前需求适用开发验收数据基线时，在 TRD 设计由本需求承担的基线增量与 ownership：定义稳定身份、幂等建立或更新、漂移处理、相对时间规则、项目实际支持的恢复方式及管理范围；恢复可按实际选择幂等重跑或补齐等非破坏方式、定向重置、可重建开发环境，不强制 destructive reset；同时涉及 DB 与 S3/对象存储时定义跨存储一致性，并明确交付文档更新与真实验证。当前需求不影响基线时，不机械增加基线章节或虚构设计。
