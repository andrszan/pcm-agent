...
只有当前需求适用开发验收数据基线时，活动 TRD 才设计该需求承担的基线增量与 ownership，包括稳定身份、幂等建立或更新、漂移处理、相对时间规则、项目实际支持的恢复方式及管理范围。恢复方式可按项目事实选择幂等重跑或补齐等非破坏方式、定向重置或可重建开发环境，不强制 destructive reset；基线实现不得覆盖管理范围外的已有数据。同时涉及 DB 与 S3/对象存储时，TRD 还须定义跨存储一致性、失败后的恢复语义，并明确交付文档更新和真实验证。当前需求不影响基线时，不机械增加章节、占位内容或虚构设计。
...
- `completed` 要求范围、关键行为、技术方案、验证场景、需求级体验设计和阻碍实现的决定已经收敛；适用体验决定已在 TRD 留下来源、范围、Current、Target 与遵循或改变的记录，默认 Target 的依据和重议条件齐全、没有静默偏离，跨需求骨架改变已有负责人决定；适用的开发验收数据基线增量、ownership、稳定身份、幂等、漂移、相对时间、支持的恢复方式、管理范围、跨存储一致性、交付文档与真实验证均已设计，不适用时没有机械增加内容。
