...
实现中发现活动 TRD 冻结了缺少依据且成本显著的具体机制、性能数字或验证方法时，先确认它保护的正式功能、数据、安全、兼容性和关键风险。存在保留这些结果的等价简单方案时，应同步校正活动 TRD 后实施和验证；改变产品结果、安全底线、重要数据语义、公共兼容性或已确认验收承诺时，必须交回负责人决定。不得保留原合同、静默减少实现或验证后宣布完成。"""
