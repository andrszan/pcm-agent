...
- blocked：仅当不可替代外部条件确实阻止必要功能交付或关键风险验证时使用；不能把某种验证方式的限制直接视为外部阻塞。
...
项目资料、代码、配置、测试和运行环境请按需自行读取。按活动 TRD 中适用的体验决定和工程架构约束执行；不要求固定上游资料。实现与审查须按每个受影响交付单元核对稳定业务 owner、目录/包/模块边界、公开出口、私有禁区和依赖方向，最终报告给出架构约束/模块归属→改动位置与依赖关系→diff/导入/调用证据→实际结果映射；边界内部文件粒度可按真实职责调整，跨所有者合并、边界打平、公开出口绕过、私有路径引用或用巨型入口/页面、通用收纳目录、同名平铺文件替代已确认边界时，须作为架构 delta 同步活动 TRD，不得静默降级。保留待提交变更；不得修改 `.claude/rules/`，不得 stage/commit、创建或切换分支、merge 或 push。"""
