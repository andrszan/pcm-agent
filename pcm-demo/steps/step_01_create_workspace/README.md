...
最终目录尚未发布时，恢复不续接旧 clone 或准备现场。程序先使用已有 `state.workspace`、当前 run ID 和当前配置的工作区根重新计算并核对精确 staging 路径；只有该路径已由本 run 记录、仍位于相同根目录且是非符号链接的真实目录时，才清理残留，同时删除旧 `template` 和 `checks` 证据，再从模板完整 clone、准备和发布。`git clone` 失败或超时留下的目录，以及删除上游 `.git/` 后初始化中断的目录，都按此规则在同一 run 重建。首次遇到尚无本 run 工作区记录的同名目录、符号链接、普通文件或配置/路径不一致时一律拒绝并保留现场。

最终目录已经发布时保留原恢复逻辑：程序不清理或重建产品目录，只在发布证据、根 `.env` 和当前配置源一致时补齐或核验零提交根仓库；临时和最终目录同时存在、最终目录归属不明或内容冲突时失败并保留现场。已发布后的恢复只使用工作区副本，不重新读取、同步或比较已删除/漂移的外部资料源。只有 SSH 读取权限缺失属于 `blocked`；工作区根位于能力仓库内部、目录冲突、初稿选题不明确、Agent 工作区配置缺失/无效/漂移、模板未忽略 `.env`、API、解析、Git、哈希、清理、发布或根仓库初始化错误属于 `failed`。已经推进到后续步骤的旧 run 不回退，也不由其它步骤自动补写根 `.env`。
