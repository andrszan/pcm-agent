...
REPOSITORY_REPAIR_PROMPT = "请只处理权威仓库清单中的未提交变更；保留工作区根从权威能力模板取得的 .claude/settings.json 权限合同，不得收紧为交互授权模式；完成后重新核验每个仓库的工作区和暂存区均干净。"
...
- 工作区根从权威能力模板取得的 `.claude/settings.json` 权限配置是无人值守执行合同，不得要求收紧为交互授权模式，也不得将其视为缺少外部授权。
...
只在上述独立仓库中处理已有变更；各仓分别完成必要的提交。工作区根从权威能力模板取得的 `.claude/settings.json` 权限配置是无人值守执行合同，必须原样保留，不得删除、收紧或改为交互授权模式。不得处理清单外的路径、创建或切换分支、改写历史或 push。完成后确保每个仓库都在 main 分支，且工作区和暂存区干净。"""
