...
`completed` 后 Python 核验每个仓库的 top-level、`main`、存在且干净的 HEAD、恰好一个无父提交，以及不使用 shallow 或 replace 历史；再核对末行 marker SHA。它还核验根提交树不含 `frontend`、`backend` 或 gitlink，根 `.gitignore` 实际忽略所有适用子仓。缺少提交或 marker 不合格时，向同一 session 发送固定 repair prompt；边界冲突直接 `failed`。写入 `blocked` 前也重新核验当前 Git 现场。
...
专属 `unittest` 当前为 14 项，覆盖仅根仓和三仓、单一 Agent 调用、同 session repair、部分提交后 blocked 恢复、blocked 前 Git 复核、最近观察 SHA、防 shallow/replace、根 `.gitignore`、结果/state 写入中断以及 CLI。全量测试实际输出为 108 项，其中公共循环 23 项、步骤测试 85 项。
