...
>
> Demo 预期以一天级实现和首次运行为目标，允许代码朴素、只支持一个本地样例、不追求正式系统的通用性和可靠性。
...
一天级 Demo 默认只承诺实现 PRD 中“E.1 当前必须实现”的首版范围。“E.2 重要增强”和“E.3 未来扩展”不得自动进入首轮 Backlog，除非它们是完成 E.1 闭环不可缺少的条件。
