...
- `run_all.py` 已按 state 顺序启动单步子进程，复用第 0～18 步及需求循环，阶段一完成或失败停止时展示独立计时汇总；
...
当前 `run_all.py` 已串联第 0～18 步并持续处理阶段一需求，阶段二仍未实现。单步和完整运行共用 `run_step.py` 重试循环外层的计时：默认在 stderr 显示每步开始、本次结束和累计运行耗时；完整编排完成或中途停止时显示本 run 汇总，不修改原 stdout 结果路径。

累计运行耗时覆盖 Agent、负责人决策、工具与验证、程序收口、自动重试及其等待，不包含进程退出后等待修复或资源的间隔；另保留首次开始与首次成功结束时间，计时完整时显示包含停机间隔的总历时。第 0～12 步按项目、第 13～18 步按需求 ID 和步骤编号区分。失败和阻塞保留耗时，恢复新增记录并累计；成功复用检查单独标识，不覆盖原始完成耗时；不适用跳过仍记录实际检查耗时。

计时独立存放于 Git 忽略的 `runs/<run-id>/timings.json`，不是业务 result/state 或需求生命周期的一部分。历史未记录、强制结束缺少结束记录和中途接入旧运行均保持未知或不完整，不通过文件时间、run ID 或恢复时间补造；计时读写失败只警告，不改变业务退出码。正在运行的进程不热加载，后续新子步骤自然生效，不为计时打断现有执行。详细字段和恢复计量规则见活动 TRD 的“步骤计时合同”，使用入口见 `pcm-demo/README.md`。
