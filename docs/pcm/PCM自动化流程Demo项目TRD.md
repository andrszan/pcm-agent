...
>
> 重构前，技术探针以及业务第 0～7 步已真实运行。第 8 步当前合同已在 `step01-mendmark` 完成真实验证。第 9 步生成并精确提交固定工程架构文档，独立核验与幂等重跑通过。第 10 步代码、自动化和 fresh 真实运行也已完成：公共 prompt 继续采用 `role/project_context/responsibility/completion/output` 五段，适用时固定产品级 UI/UX 框架文档已精确提交，`steps/10.json` 与 state 均为 success 并推进第 11 步。旧 403、非 JSON、非法 completed 和旧 prompt 设计只保留为已修复的根因历史；`step08-real-20260823-a/b` 继续仅为旧第 8 步合同历史。
...
3. **技术阶段 2：第 3～11 步**——第 3～10 步已实现并真实验证，第 11 步待逐步实现。按新版顺序完成基础工程选型、组装、准备核验、项目化、总体技术方案、仓库首次提交、必要工程架构、按需 UI/UX 框架、Backlog；
...
- `run_step.py` 的第 0～10 步单步入口；其中只对 schema 完整的第 8～10 步 success 保护既有 result 和已推进 state，后续重跑失败不覆盖 success、不回退节点，残缺 success 不保护，其它步骤保持原行为；
- 第 0 步完整产品初稿无副作用跳过；
...
第 0～10 步均已完成代码、自动化和适用真实验证；未实现能力必须返回明确程序错误，不得以空脚本、固定 JSON、旧实现或口头结论冒充成功。

### 3. 本轮实现与同步边界

本轮按照“先代码与自动化测试，再补真实集成验证和文档事实”的顺序完成：

- 第 8 步的权威多仓只读 Git 核验与干净基线、第 9 步的固定工程架构文档边界、tracked 核验和提交执行锚点、第 10 步的确定性适用性、固定 UI/UX 文档边界与提交锚点均保持步骤私有，不扩展为公共 Git DSL，也不抽取通用 commit 能力；
...
- 公共循环 24 项、第 10 步本体 16 项与 CLI 5 项、第 9 步专属 29 项、第 7 步 8 项、第 8 步 17 项，相关定向 99 项、从 `pcm-demo/` 根发现 `common/` 与 `steps/` 的全量 162 项测试均已通过；`compileall`、`git diff --check` 和新增 README 的 no-index whitespace 检查均已通过，IDE 对第 10 步 `step.py`、`test_step.py`、`test_cli.py`、`run_step.py` 无诊断；
...
- 第 9 步旧失败依次暴露 HTTP 403、非 JSON 普通文本和非法 `completed` 字段组合；用户重构公共五段 prompt 并清理局部历史后，fresh 运行已生成、修正并提交固定工程架构文档，最终 decision、state、Git 核验和幂等重跑全部成功；第 10 步随后以同一公共循环完成适用路径的固定 UI/UX 框架文档、提交、状态推进与幂等重跑；
- Git 忽略 run `prompt-role-replay-20260823` 保留旧四段 XML prompt 的历史回放，不再作为现行 prompt 证据；现行五段 prompt 已由第 9 步 fresh 真实运行验证；
- 当前验证记录：全量 162 项、相关定向 99 项、`compileall`、`git diff --check` 与新增 README 的 no-index whitespace 检查均通过，IDE 对第 10 步 `step.py`、`test_step.py`、`test_cli.py`、`run_step.py` 无诊断；第 8～10 步真实验收完成。本轮文档同步不替代后续步骤验证。

### 4. 本阶段继续不做

- 不一次创建第 11～19 步或阶段二节点的空壳脚本；
...
- `run_step.py`：当前运行已经实现的第 0～10 步；实际命令和配置说明以 `pcm-demo/README.md` 及各步骤 README 为准；
...
第 0～10 步结果继续使用统一的步骤字段；`phase/current_node` 作为运行状态中的恢复位置保存，不重复写入每个步骤结果。第 8 步成功时额外保存 `applicable_repositories` 和 `repositories`，而其 `outputs` 固定为空；第 9 步始终 `applicable: true`，成功 `outputs` 固定为 `docs/design/工程架构设计.md`；第 10 步 success 严格为 `applicable: true` 加唯一 `docs/ui-ux/framework.md`，或 `applicable: false` 加空 `outputs`。
...
项目上下文范围固定为：第 2 步初稿原文和目标路径；第 5 步产品定义原文、适用工程、选型白名单投影以及资源清单的绝对路径/可读性；第 6、7 步产品定义原文、准备清单原文、适用工程和组装白名单投影；第 8 步有序权威仓库相对路径和干净基线说明；第 9 步两份产品定义原文、准备清单原文、总体技术方案原文、有序权威工程和固定输出路径；第 10 步适用时还包括固定工程架构文档与实际 `frontend`。资源清单正文和 `.env` 因输入来源边界不读取或内联到项目上下文；第 8～10 步其它只读 Git 核验和 verifier 仍由步骤私有实现。
...
当前状态：较早阶段的第 4 步专属 12 项、当时第 0～4 步 44 项测试及 Python 语法检查、`compileall`、`git diff --check` 均通过；当前全量计数以本轮实际 162 项为准。修迹 run 使用 GitLab SSH 成功组装前端 `vite-react-shadcn-spa`（`main` SHA `a31db6deb85ab29f2d2253413dd362293a96325f`）和后端 `fastapi-sqlalchemy-postgresql-async-api`（`main` SHA `49ff842fcd330387f2fbdd1e9a43884e05894697`），均与远端 `main` 一致；最终目录的适用端各为自身 top-level、`main`、unborn HEAD、空 index，临时目录清理，根仓仍是零提交 `main`，黄金初稿的既有 SHA 事实未变化。首次错误 HTTPS 来源运行返回 `failed` 且未修改目标；修正为 SSH 后同 run 依据 marker 安全恢复成功，再次运行幂等复用。
...
**当前自动化事实。** 公共循环 24 项、第 5 步 10 项以及当前全量 162 项测试已经通过；公共循环已在第 7、9、10 步取得真实集成证据。上述第 5 步历史仍是旧协议事实，本文不把它改写为第 5 步独立新 session 运行。
...
**本次重构自动化事实。** 第 6 步 9 项与当前全量 162 项测试已经通过；公共循环已在第 7、9、10 步取得真实集成证据。第 6 步的真实项目化记录仍属于旧协议，本文不把它改写为第 6 步独立新 session 运行。
...
**重构前真实运行事实。** 第 7 步旧协议的自动化记录为 11 项专属及当时第 0～7 步全量测试；其后当前全量已更新为本轮实际 162 项。`compileall`、`git diff --check` 和独立只读复审通过。修迹真实运行使用 session `dd29666c-22d7-4a9f-b51b-3f6e1cf10938`；Agent 读取两份产品定义、项目准备清单、前后端工程和脱敏组装 commit 事实，生成 `docs/design/技术方案.md`，专属裁决为 `completed`，状态推进到 `project:08_initialize_repositories`。真实 run 重跑确认成功幂等复用，不再次调用 Agent 或决策模型；根仓仍为零提交 `main`，未执行 Git 写操作。
...
| 11 拆分 Backlog | `requirement-breakdown` 消费第 10 步 `status`、`applicable`、`outputs` | Backlog 覆盖最终范围，详情卡清楚，首条验证切片由完整正式需求组成；由 `commit-changes` 精确提交根仓库后进入阶段一 |
...
自动化当前基线为第 10 步本体 16 项和 CLI 5 项、第 9 步 29 项、公共 24 项、第 7 步 8 项、第 8 步 17 项、相关定向 99 项、全量 162 项；`compileall`、`git diff --check` 与新增 README 的 no-index whitespace 检查通过，IDE 对第 10 步 `step.py`、`test_step.py`、`test_cli.py`、`run_step.py` 无诊断。
...
`completed` verifier 先 repair 缺失或空文档，之后无论有无 diff 都在原 session exact 发送一次 `/commit-changes`。提交锚点要求 exact prompt、紧邻非空 Agent `user` 回复和原 session。最终文档必须非空、普通、非符号链接且 tracked；所有权威仓库均为自身 top-level、`main`、clean；Python 只读 Git。fresh/resume/blocked/idempotency/symlink/success 写入中断与第 9 步同构，局部实现，不改 `common`，不抽 commit DSL。`run_step.py` 对完整第 8～10 步 success 保护既有 result 和已推进 state；第 10 步只接受 true 加固定唯一 output 或 false 加空 outputs，残缺 success 不保护。

自动化已通过第 10 步本体 16 项与 CLI 5 项，共 21 项；全量 162 项，公共循环和第 9 步回归已单独通过。独立审查曾发现无 frontend 的 failed、blocked 或已推进状态遗留执行产物会误报 skip success，现已修正为所有状态统一拒绝并补测试。
...
第 11 步必须从第 10 步 `status`、`applicable`、`outputs` 消费交接，不能从默认路径、目录扫描或 Agent 文字猜测；第 11 步尚待实现。
...
第 0～10 步同时使用 `current_step` 和必要的 `phase/current_node`。第 9、10 步 success 均以各自固定 commit prompt 紧邻非空 Agent 回复和原 session 作为执行锚点，再以固定文档 tracked、全仓 clean 兜底；第 10 步不适用 success 则必须为 `applicable: false`、空 outputs 且没有执行产物。`run_step.py` 只对 schema 完整的第 8～10 步 success 保护既有 result 和已推进 state；后续重跑失败不会覆盖 success 或回退节点，残缺 success 不保护，其它步骤保持原入口行为。当前真实 state 为 success，位于 `project:11_requirement_breakdown`；第 10 步幂等重跑已经确认不会重复 Agent、decision 或提交。
...
10. 第 17 步状态同步提交是第 18、19 步恢复锚点；
11. 第 18 步只接受 `no_change` 或 `committed`，且只允许根仓库 `.claude/rules/` 变更；
12. 第 19 步以根 `main` 合并提交和所有适用仓库检查作为完成证据；
13. 阶段二每轮审计记录 `version_fingerprint` 和独立 session；相同清楚指纹恢复既有审计，不重复制造候选；
14. 候选分流和入池保存稳定证据；已入池候选不得重复分配 ID；
15. 阶段二修复需求临时使用 `phase_2_requirement_remediation` 和 `requirement_cycle`，并设置 `return_node_after_completion: phase_2:regress_and_reaudit`；
16. 每一步实际输入从前一步结果读取并核验，不从固定路径猜测；
17. 第 1 步继续使用现有发布阶段事实安全恢复；
18. 路径、目录、分支、提交、合并、版本指纹、候选归属或入池证据冲突时保留现场并返回 `failed`。

第 3～8 步的历史和真实验证事实保持不变。第 9 步旧失败已定位为公共 prompt 结构问题并修复；fresh 运行使用 session `f41fc439-4c46-434f-b3e9-d15c18c89601` 完成 11 条 conversation、固定文档提交、合法最终 decision、状态推进、独立 Git 核验和幂等重跑。第 10 步随后使用 session `2d052d9a-b9fd-4fcd-b5f9-ff724f25dbc1` 完成固定 UI/UX 文档、提交、独立核验和幂等重跑，当前已推进第 11 步。
...
- 第 7 步 8 项、第 8 步 17 项、第 9 步 29 项、第 10 步本体 16 项与 CLI 5 项及公共循环 24 项，相关定向共 99 项；
- 从 `pcm-demo/` 根递归发现 `common/` 与 `steps/` 的全量 162 项 `unittest` 已通过；
- Probe C 和旧四段 XML 回放只作为历史；现行五段 prompt 已由第 9、10 步 fresh 真实运行验证；
...
- 当前验证记录：`compileall`、全量 162 项 `unittest`、相关定向 99 项、`git diff --check` 与新增 README 的 no-index whitespace 检查均已通过；IDE 对第 10 步 `step.py`、`test_step.py`、`test_cli.py`、`run_step.py` 无诊断。

### 2. 后续增量风险

进入对应步骤后再增加最小真实验证：

- 第 11 步：Backlog 完整覆盖、消费第 10 步结果和提交；
...
3. `run_all.py` 在何时建立，以及第 0～10 步现有入口如何与新节点协议复用；
4. 第 11 步如何消费第 10 步 `status`、`applicable`、`outputs`，以及 Backlog 完整覆盖、提交、失败、阻塞、恢复和验证方式；
...
这些未决事项不改变第 0～10 步已经完成的真实事实。第 11 步及后续每个新步骤继续对照原手稿、当前流程设计和活动 TRD 与开发者确认详细合同，确认后先实现代码并通过测试和真实验证，再将实际实现同步到设计文档并提交。
