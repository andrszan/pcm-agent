...
>
> PCM Demo 当前工作树全量 319 项 `unittest`、`compileall common steps run_step.py test_run_step_retry.py` 与 `git diff --check` 通过（全量耗时 54.766 秒）。当前工作树还包含其它公共循环/CLI 的未提交修改，故该全量结果不能全部归因于第 9 步。`run_step.py` 只对显式运行时请求有界重跑当前步骤两次，间隔 10 秒和 30 秒；当前请求仅由 Claude Agent SDK 实际执行通道故障产生，任意 API 状态码均可重试，业务 `blocked`、普通 `failed`、AI-compatible 裁决失败、本地合同错误和取消不重试。一次性请求不持久化，18 个步骤不增加重试分支。
...
- 第 9 步始终 `applicable: true`，固定读取第 2 步两份产品定义、第 5 步准备清单、第 7 步总体技术方案和第 8 步严格 success 的空 `outputs`、result/state 一致的合法有序 `applicable_repositories`，在单一 `engineering_architecture` session 中形成 `docs/design/工程架构设计.md`。Agent 必须以 `[当前]`、`[目标]`、`[按需]`、`[迁移]` 为每个权威工程建立有限地图，使用有证据的架构决策矩阵收敛模块职责、语义所有权、公开/私有边界、依赖与共享准入、代表性文件放置、最小迁移、可观察演进和当前下游所需的高影响决定；不得为 MVC、分层、六边形或 DDD 预建无当前消费者的层、服务、队列、接口或共享结构。repair 后仅在根仓存在且仅存在固定文档未提交变化时才在原 session 调用 `/commit-changes`，否则已 tracked 且全仓 clean 即完成。Python 不解析这些 Markdown 语义，只核验固定文档与当前 Git 边界；
...
第 9 步当前 prompt 合同已完成代码与自动化，但尚未执行安全的 fresh 真实 Claude Agent、AI-compatible 负责人或 `/commit-changes` 集成；第 10、11 步业务合同未因本轮共享统计同步而改变。第 0～8、12～18 步已完成代码、自动化和适用真实验证；未实现能力必须返回明确程序错误，不得以空脚本、固定 JSON、旧实现或口头结论冒充成功。
...
- 调用方已授权为当前自动化开发周期建立唯一资源准备基线并创建 `docs/requirements/项目准备清单.md`。Agent 可以原样读取任意格式的可信开发资源资料，从产品范围推导编码、开发环境联调和开发环境真实验收所需、且必须由调用方提供的外部服务、账号、凭据、授权素材、私有数据或专用设备；在授权和配置合同允许时创建项目专用开发/测试资源与最小权限运行凭据。每项判为 `ready` 的外部运行资源都必须把最终项目凭据与资源绑定持久化到所属仓库被 Git 忽略的实际 `.env` 或等价受保护配置，后续开发无需重新读取共享资源资料；同步无秘密 `.env.example` 或公开说明，POSIX 上含秘密文件通常为 `0600`，并使用最终项目运行凭据完成最小行为和隔离验证。不得泄露秘密、创建生产或未授权付费资源、改变既有 Git 边界，或执行 Git 暂存、提交、分支、合并、push；
- 只允许两类 `blocked`：开发必需外部资源在候选池中不存在、当前环境无法安全生成且无兼容替代；或已匹配资源真实不可用、凭据无效、权限不足、隔离不合格，或缺少开发所需接口能力、可用配额、回调/白名单、沙箱范围及其它既定能力。依赖安装、构建、测试、migration、Seed、业务实现、项目内测试账号、完整联调和浏览器验收属于后续开发工作；生产部署、正式域名、DNS/TLS、生产凭据、监控、备份恢复、容量和发布安全属于开发范围外；产品规则和隐私事项由后续设计收敛。上述事项的 `missing` / `pending` 不得阻塞第 5 步；
- 每轮从 init 核验实际 cwd、`project-readiness` Skill 和 slash command；完整真实回复先保存为 `user`，再由公共渲染器以步骤 `DECISION_RULES` 和项目上下文生成的完整 XML system snapshot 的统一 `AgentDecision` 裁决。`completed` 必须确认开发必需外部资源均真实可用或明确不需要，所有 `ready` 外部资源的最终凭据绑定已持久化到项目受保护配置并同步公开键合同，含秘密文件权限安全，且不存在两类开发资源阻塞；清单不能为自身状态作证。生产发布、后续业务实现和产品规则待定不得触发 `continue` 或 `blocked`。`completed` 后程序重新核验清单、交接和根/适用子仓 Git 边界；清单缺失或为空时追加固定修复提示，只继续开发资源准备、凭据持久化、公开示例、文件权限和最终运行凭据验证；
...
- 成功先写 `steps/05.json`，其中 `outputs` 仍只含固定准备清单路径，`readiness_baseline` 记录当前清单与两份权威产品定义的 SHA-256；该无秘密指纹只保护已经完成的基线不被静默替换，不证明资源 `ready`，也不记录或哈希 `.env`、凭据和资源资料正文。随后推进到 `project:06_bootstrap_foundation`。若结果已成功而下一节点状态写入中断，重跑以严格成功 schema、当前指纹、交接和 Git 边界恢复；缺少指纹的旧 success、清单漂移或产品定义漂移均不可复用或自动升级；
...
**当前合同真实验证。** run `step05-readiness-v2-20260828-b` 在独立产品工作区完成第 0～4 步后真实调用 `/project-readiness`，依次暴露并保留三类失败现场：外部资源标记 `ready` 但 MinIO/SMTP 凭据未持久化且 `.env` 为 `0644`；生产/发布条件和业务规则待定被错误扩大成开发阻塞；内部配置键不清可能形成第三类 blocked，且资源基本连接通过但开发必要能力、配额、回调/白名单不足未明确归入第二类。最终合同将 blocked 严格限定为：开发必需外部资源缺失、当前环境无法安全生成且无兼容替代；或已匹配资源不可用、凭据无效、权限/隔离不合格，或缺少开发所需能力、配额、回调/白名单和其它既定能力。最终 fresh Agent 将 PostgreSQL、MinIO、SMTP 共 24 个运行字段持久化到被忽略且 `0600` 的 `backend/.env`，同步无秘密 `.env.example` 和前端公开地址配置；真实探针确认 PostgreSQL 开发/测试库事务性读写、管理库连接拒绝、非特权角色属性，MinIO 仅列出两个项目桶且完成读写删除、3 个范围外桶拒绝，SMTP 完成 TLS、认证和唯一授权收件人单封投递；秘密扫描为 0。清单明确“当前开发资源阻塞：无”，将依赖、migration、Seed、业务实现、项目内账号、完整联调和浏览器验收归为后续开发，把生产与规则事项归为非阻塞。最终 conversation 为 `system → assistant → user → assistant completed`，`steps/05.json` 严格 success 且 baseline 匹配，state 推进第 6 步；0.967 秒幂等重跑三份运行文件字节不变。
...
现行 initial prompt 与 `ENGINEERING_ARCHITECTURE_DECISION_RULES` 要求 Agent 为每个权威工程给出有限 Current/Target 地图，并以 `[当前]`、`[目标]`、`[按需]`、`[迁移]` 标注。完成裁决必须基于有证据的架构决策矩阵：目录/模块职责、语义所有权、稳定公开能力、私有禁区、允许/禁止依赖和共享准入均须明确；文档还须包含 2～5 个代表性文件放置演练、最小迁移与可观察演进，并收敛当前下游实际需要的高影响决定。MVC、分层、六边形和 DDD 不是互斥四选一，不得为任一范式预建 `domain`、`application`、`infrastructure`、`shared`，或预建没有当前消费者的服务、队列、接口或其它结构。配置、运行、数据、测试和安全只按当前证据展开，禁止虚构 `[当前]`。
...
现行实现本体 9 项、CLI 4 项；公共循环与第 17 步定向 43 项通过。当前工作树全量 319 项 `unittest` 通过（54.766 秒），`compileall common steps run_step.py test_run_step_retry.py` 与本次目标 `git diff --check` 通过；当前工作树还包含其它公共循环/CLI 的未提交修改，故全量结果不能全部归因于第 9 步。独立只读审查无高、中置信发现。BR-001 旧运行有完整四段 conversation；BR-002 在 direct-run 版本期间只有 Claude session、没有 decision reference 或 conversation 文件，该历史缺口不补造，未来需求按现行合同保存完整历史。
...
- 第 0～8、12～18 步状态转换；
- 第 5 步本体 14 项、第 6 步本体 9 项、第 5/6/7/9/10/11 步相关回归 81 项、第 13 步本体 20 项与 CLI 10 项、显式 SDK 通道重试 7 项均保留其专属验证语境；本轮当前工作树全量 320 项 `unittest` 通过（61.405 秒），`compileall common steps run_step.py test_run_step_retry.py` 与本次目标 `git diff --check` 通过。当前工作树还包含其它公共循环/CLI 的未提交修改，故全量结果不能全部归因于第 9 步；
- 重试定向覆盖任意 API 状态、明确 `api_error`、SDK 连接/进程异常、普通失败、取消、本地合同错误、内部退出码耗尽归一和瞬时标志不落盘；本次未调用真实外部服务制造故障；
- 第 5 步自动化覆盖当前开发资源 Prompt/决策规则、严格两类 blocked、必要能力/配额/回调/白名单、生产与后续实现非阻塞、完成后 `readiness_baseline` 写入、清单漂移、两份产品定义漂移、缺少指纹的旧 success 拒绝和幂等复用；真实 run `step05-readiness-v2-20260828-b` 已完成 PostgreSQL/MinIO/SMTP 最终凭据持久化和探针、负责人 `completed`、success baseline、状态推进与字节级幂等验证；
