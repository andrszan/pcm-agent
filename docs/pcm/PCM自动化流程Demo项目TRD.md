...
>
> 本轮已建立自包含决策交接与多轮 `continue` 兜底：模板根 `AGENTS.md` 约束 Agent 在确需负责人决定时完整说明准确问题、已核验事实与约束、实质可行选项、主要影响及推荐理由，引用只辅助定位；`common/decision.py` 的统一负责人职责在新 conversation 中拒绝引用式或信息不足的交接，要求原 Claude session 重新读取并补齐，不得猜测、`completed` 或把信息不足判为 `blocked`。`dev-workflow` 最终回复必须显式声明三种结束状态之一，第 15 步仅在 Agent 明确声明已完成且没有剩余工作、失败项、验证缺口或阻断项时 `completed`。公共循环、`AgentDecision`、conversation/state、completion verifier、项目上下文范围、历史 run 和外部产品工作区均未修改，也未引入产物全文注入、动态快照、文件读取工具或自然语言解析。定向 57 项通过；全量 369 项中 368 项通过，唯一失败是当前 `HEAD` 已存在的第 17 步轮数常量与测试期望不一致；完整 `compileall`、eval JSON 解析和 `git diff --check` 通过。fresh Probe C 一次确认引用式交接为 `continue`，一次确认自包含不可替代外部资源缺口为 `blocked`。新规则不迁移既有 decision conversation，Agent 侧模板规则只随未来新建工作区生效。
>
...
- 第 0～8、12～18 步状态转换；
- 本轮自包含决策交接合同由公共循环与第 15 步定向 57 项覆盖，包括新负责人 XML prompt 的交接充分性要求、`continue.answer` 原样恢复同一 session、历史 system snapshot 不被覆盖、第 15 步显式完成状态和简短正向完成报告；当前工作树全量 369 项中 368 项通过（54.112 秒），唯一失败为本次未修改且当前 `HEAD` 已存在的 `steps.step_17_commit.test_step.RequirementCommitTests.test_dirty_completed_creates_full_conversation`，实现 `MAX_DECISION_ROUNDS=32` 而断言仍期望 16。完整 `compileall common steps probes run_step.py run_all.py test_run_step_retry.py test_run_all.py`、dev-workflow eval JSON 解析和 `git diff --check` 通过；fresh Probe C `probe-c-decision-handoff-20260905` 的引用式不完整交接与自包含阻塞两项检查均一次通过；
