...
>
> 重构前，技术探针以及业务第 0～7 步已真实运行。第 8 步当前合同已在 `step01-mendmark` 完成真实验证。第 9 步生成并精确提交固定工程架构文档，独立核验与幂等重跑通过。第 10 步按确定性适用规则生成并提交固定产品级 UI/UX 框架。第 11 步代码、自动化和 fresh 真实运行已经完成：固定 Backlog 已精确提交，文档不含需求开发状态。该历史 `steps/11.json` success 当时进入旧 `phase_1:select_requirement` / step 12 占位入口；第 12 步真实运行前严格核验旧 state，仅 run-local 规范化为 `phase_1:initialize_requirement_registry`，生产代码未加入旧入口兼容。第 12 步已用真实 Responses 初始化 14 项 BR 注册表；第 13 步已确定性选择 `BR-001`、建立三仓 `req/br-001` 并推进到 `requirement:14_trd_design` / step 14。第 14 步及以后循环仍未实现。旧 403、非 JSON、非法 completed 和旧 prompt 设计只保留为已修复的根因历史；`step08-real-20260823-a/b` 继续仅为旧第 8 步合同历史。
...
4. **技术阶段 3：循环前第 12 步与阶段一第 13～18 步**——第 12、13 步代码、自动化和真实验证已完成；第 14 步开始仍须逐步跑通活动 TRD、实现验证、复盘、提交和合并；
...
- `run_step.py` 的第 0～13 步单步入口；第 8～12 步保留固定 result success 保护，第 13 步以 current `active_requirement`/cycle 定位并保护唯一完整 scoped success；
- 第 0 步完整产品初稿无副作用跳过；
...
- 第 14～18 步的代码、测试和真实运行；
- `run_all.py` 完整串联入口；
- 第 14～18 步所需的 TRD、session、提交与合并持久化。

第 0～13 步均已完成代码、自动化和适用真实验证；未实现能力必须返回明确程序错误，不得以空脚本、固定 JSON、旧实现或口头结论冒充成功。
...
- 第 12 步本体 11 项与 CLI 6 项、第 13 步本体 19 项与 CLI 9 项、两步定向 45 项及全量 222 项测试均已通过；`compileall`、`git diff --check` 通过，IDE 对第 12、13 步和 `run_step.py` 无诊断，独立审查 3 项中置信问题已修复，复核无高、中置信发现；Ruff 未安装，未执行 Ruff；
...
- 当前验证记录：第 12 步本体 11 项与 CLI 6 项、第 13 步本体 19 项与 CLI 9 项、两步定向 45 项和全量 222 项 `unittest`、`compileall`、`git diff --check` 均通过；IDE 对第 12、13 步与 `run_step.py` 无诊断，独立审查 3 项中置信问题已修复，复核无高、中置信发现；第 8～13 步真实验收完成。本轮文档同步不替代后续步骤验证。
...
- `run_step.py`：当前运行已经实现的第 0～13 步；实际命令和配置说明以 `pcm-demo/README.md` 及各步骤 README 为准；
...
自动化当前基线为第 12 步本体 11 项与 CLI 6 项、第 13 步本体 19 项与 CLI 9 项、两步定向 45 项、全量 222 项；`compileall`、`git diff --check`、IDE 通过，独立审查 3 项中置信问题已修复，复核无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
自动化已通过第 10 步本体 16 项与 CLI 5 项；当前第 12 步本体 11 项与 CLI 6 项、第 13 步本体 19 项与 CLI 9 项、两步定向 45 项、全量 222 项通过。独立审查曾发现无 frontend 的 failed、blocked 或已推进状态遗留执行产物会误报 skip success，现已修正为所有状态统一拒绝并补测试。
...
成功 `steps/11.json` 严格为 `applicable: true` 和唯一 output；第 11 步自身的 success 交接进入 `phase_1_requirement_development` / `phase_1:initialize_requirement_registry`，`step/current_step` 为 12，`active_requirement`、`requirement_cycle` 为 `null`，且不写需求注册表。当前 `run_step.py` 已支持第 0～13 步：完整第 8～12 步 fixed success 与当前 active requirement 的第 13 步 scoped success 分别受保护。
...
自动化通过第 12 步本体 11 项与 CLI 6 项、第 13 步本体 19 项与 CLI 9 项、两步定向 45 项、全量 222 项；`compileall`、`git diff --check` 通过，IDE 对第 12、13 步与 `run_step.py` 无诊断，独立审查 3 项中置信问题已修复，复核无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
### 阶段一：已实现第 12、13 步；第 14～18 步循环待实现

第 12 步位于需求循环之外；第 13 步已完成首个需求选择和统一建分支，`step01-mendmark` 当前停在尚未实现的第 14 步。第 14～18 步仍是当前已知正式 Backlog 的未来循环。
...
第 0～13 步同时使用 `current_step` 和必要的 `phase/current_node`。第 9～11 步 success 均以各自固定 commit prompt 紧邻非空 Agent 回复和原 session 作为执行锚点，再以固定文档 tracked、全仓 clean 兜底；第 10 步不适用 success 则必须为 `applicable: false`、空 outputs 且没有执行产物。`run_step.py` 对 schema 完整的第 8～12 步 success 保持固定保护；第 13 步只对当前 active/cycle 的完整 scoped success 保护 result/state，残缺或失败 result 可恢复覆盖。

当前真实 state 位于 `phase_1_requirement_development` / `requirement:14_trd_design`、`step/current_step: 14`。注册表含 14 项 BR：`BR-001` 为 active、13 项 pending、0 项 completed；`active_requirement` 仅为 `BR-001`，cycle 保存 `req/br-001`、各仓 `base_sha` 和 `return_node_after_completion`。第 13 步完整 scoped result 的幂等重跑已经确认不调用模型、Claude session、负责人决策或 Git 写操作，result/state/ref 均不变。第 14 步及以后仍未实现；历史第 11 步 success 进入旧 step 12 占位入口的事实仅用于第 12 步真实 run 前的严格 run-local 规范化，不是生产入口兼容能力。
...
- 第 12、13 步定向共 45 项；从 `pcm-demo/` 根递归发现 `common/` 与 `steps/` 的全量 222 项 `unittest` 已通过；
...
- 当前验证记录：`compileall`、全量 222 项 `unittest`、第 12/13 步定向 45 项、`git diff --check` 均已通过；IDE 对第 12、13 步和 `run_step.py` 无诊断，独立审查 3 项中置信问题已修复，复核无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
3. `run_all.py` 在何时建立，以及第 0～13 步现有入口如何与新节点协议复用；
...
这些未决事项不改变第 0～13 步已经完成的真实事实，也不重新打开本轮已实现的第 12、13 步合同或已确认的第 14～18 步总体合同。后续从第 14 步开始；每个新步骤继续对照原手稿、当前流程设计和活动 TRD 确认实现级输入、输出、失败、阻塞和恢复细节，随后先实现代码并通过测试和真实验证，再将实际实现同步到设计文档并提交。
