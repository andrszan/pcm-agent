...
>
> 重构前，技术探针以及业务第 0～7 步已真实运行。第 8～14 步已按当前合同完成真实验证；第 14 步预持久化 exact TRD 路径，在 requirement-scoped `trd-design` session 中形成 BR-001 活动 TRD并推进 `requirement:15_development` / step 15。第 14 步前用户授权的 Skill 命名提交使 root 旧 cycle base 漂移；经用户明确选择后归档旧 run、确认旧需求分支无独有提交并从最新 `main` 重跑第 13 步。第 14 步真实运行又发现负责人把实现门槛误判为 completed，已通过同 session 语义复核收敛并收紧生产 completion 规则。第 15 步及以后仍未实现。旧 403、非 JSON、非法 completed 和旧 prompt 设计只保留为已修复的根因历史；`step08-real-20260823-a/b` 继续仅为旧第 8 步合同历史。
...
3. **技术阶段 2：第 3～11 步**——已实现并真实验证。按新版顺序完成基础工程选型、组装、准备核验、项目化、总体技术方案、仓库首次提交、必要工程架构、按需 UI/UX 框架和 Backlog；
4. **技术阶段 3：循环前第 12 步与阶段一第 13～18 步**——第 12～14 步代码、自动化和真实验证已完成；第 15 步开始仍须逐步跑通实现验证、复盘、提交和合并；
...
- `run_step.py` 的第 0～14 步单步入口；第 8～12 步保留固定 result success 保护，第 13、14 步分别以 current `active_requirement`/cycle 定位并保护唯一完整 scoped success；
- 第 0 步完整产品初稿无副作用跳过；
...
- 第 10 步仅以第 8 步 result/state 一致的 `applicable_repositories` 是否含 `frontend` 判断 Demo v1 适用性；不适用时零执行副作用，适用时在单一 `ui_ux_framework` session 中生成 `docs/ui-ux/framework.md`，completion verifier 同 session 固定调用一次 `/commit-changes`，Python 只读 Git；
- 第 11 步严格读取第 2/5/7/8/9/10 步交接，在单一 `requirement_breakdown` session 中生成唯一 `docs/backlog/backlog.md`，completion verifier 同 session fixed exact `/commit-changes`；Backlog 不记录需求开发状态，Python 只读 Git，success 进入第 12 步初始化入口；
- 第 12 步严格消费第 11 步完整 success；Responses/Pydantic 只提取 `id`、`title`、`order`、`depends_on`，ID 与依赖 ID 仅接受 `[A-Za-z0-9][A-Za-z0-9_-]*` 且 ID 忽略大小写唯一；Python 对唯一正式总览、详情区、每卡唯一 `##### 前置依赖` 与模型输出三方严格一致性、连续 order 和依赖图进行校验，确认 root 自身 top-level、`main`、clean、Backlog tracked 和有效 `HEAD` 后先写 result、再写全 pending 注册表；不调用 Claude Agent、`AgentDecision` 或 Skill，不写产品或 Git；
- 第 13 步严格消费第 8/12 步交接，零 AI/Agent/Skill 确定性选择 ready pending 需求；fresh 全局预检后先写 active intent/cycle，再仅以 `git switch -c` 在全部适用仓建立 `req/<lowercase-id>`，写 scoped result 并推进第 14 步；
- 第 14 步严格消费 current active/cycle 和第 2/5/7/9/10/11/13 步交接，fresh 全局预检全部 requirement branch/base/clean 后先持久化 exact `trd_path`，再在 requirement-scoped session 显式调用 `/trd-design`；默认 Agent 无 Bash 且 PreToolUse 只允许写 exact TRD，success 保留唯一未提交 TRD并推进第 15 步；
...
- 第 15～18 步的代码、测试和真实运行；
- `run_all.py` 完整串联入口；
- 第 15～18 步所需的开发 session、提交与合并持久化。

第 0～14 步均已完成代码、自动化和适用真实验证；未实现能力必须返回明确程序错误，不得以空脚本、固定 JSON、旧实现或口头结论冒充成功。

### 3. 本轮实现与同步边界

本轮按照“先代码与自动化测试，再补真实集成验证和文档事实”的顺序完成：

- 第 8 步的权威多仓只读 Git 核验与干净基线、第 9 步的固定工程架构文档边界、tracked 核验和提交执行锚点、第 10 步的确定性适用性、固定 UI/UX 文档边界与提交锚点，以及第 11 步的固定 Backlog、工作树边界和提交锚点均保持步骤私有，不扩展为公共 Git DSL，也不抽取通用 commit 能力；
...
- 第 12 步 17 项、第 13 步 28 项、第 14 步 20 项，第 12～14 步定向 65 项及全量 243 项测试均已通过；`compileall`、`git diff --check` 通过，IDE 对第 12、13 步和 `run_step.py` 无诊断，独立审查发现的 4 项中置信问题均已修复并补测试，最终复核无高、中置信发现；Ruff 未安装，未执行 Ruff；
- Probe C 的格式重试和 `probe-c-20260822T200038Z` 结果均保留为旧合同历史。现行 `common/decision.py` 要求步骤显式传入完整 XML system prompt，将其原样传给一次 `responses.parse` 并以 Pydantic `AgentDecision` 取得结构化输出；没有公共默认或隐藏追加 prompt，也没有格式重试。现行 prompt 已由第 9 步与第 10 步 fresh 真实运行验证；
...
- 第 9 步旧失败依次暴露 HTTP 403、非 JSON 普通文本和非法 `completed` 字段组合；用户重构公共五段 prompt 并清理局部历史后，fresh 运行已生成、修正并提交固定工程架构文档，最终 decision、state、Git 核验和幂等重跑全部成功；第 10 步随后完成适用路径的固定 UI/UX 框架文档、提交、状态推进与幂等重跑；第 11 步再以严格第 2/5/7/8/9/10 交接完成固定 Backlog、提交与幂等重跑；第 12 步对该 Backlog 完成真实 Responses 注册、状态推进与零模型幂等重跑；
- Git 忽略 run `prompt-role-replay-20260823` 保留旧四段 XML prompt 的历史回放，不再作为现行 prompt 证据；现行五段 prompt 已由第 9 步 fresh 真实运行验证；
- 当前验证记录：第 12 步 17 项、第 13 步 28 项、第 14 步 20 项，第 12～14 步定向 65 项和全量 243 项 `unittest`、`compileall`、`git diff --check` 均通过；全工作区 IDE diagnostics 无问题，独立审查发现的 4 项中置信问题已修复并补测试，最终复核无高、中置信发现；第 8～14 步真实验收完成。本轮文档同步不替代后续步骤验证。
...
- `run_step.py`：当前运行已经实现的第 0～14 步；实际命令和配置说明以 `pcm-demo/README.md` 及各步骤 README 为准；
...
项目上下文范围固定为：第 2 步初稿原文和目标路径；第 5 步产品定义原文、适用工程、选型白名单投影以及资源清单的绝对路径/可读性；第 6、7 步产品定义原文、准备清单原文、适用工程和组装白名单投影；第 8 步有序权威仓库相对路径和干净基线说明；第 9 步两份产品定义原文、准备清单原文、总体技术方案原文、有序权威工程和固定输出路径；第 10 步适用时还包括固定工程架构文档与实际 `frontend`；第 11 步包括第 2/5/7 步文档、第 8 步权威工程、第 9 步工程架构和严格第 10 步交接，仅在第 10 步 true 时读取 UI/UX 框架。资源清单正文和 `.env` 因输入来源边界不读取或内联到项目上下文；第 8～11 步其它只读 Git 核验和 verifier 仍由步骤私有实现。
...
| 10 产品级 UI/UX 框架（按需） | Demo v1 仅以第 8 步 result/state 一致的 `applicable_repositories` 是否含 `frontend` 判断；适用时 `ui-ux-framework` | 适用时固定 `docs/ui-ux/framework.md` 已 tracked、全仓 clean 并推进第 11 步；不适用时 `applicable: false`、`outputs: []` 且零执行副作用 |
| 11 拆分 Backlog | `requirement-breakdown`；同一 session 固定调用一次 `commit-changes` | 第 2/5/7 步文档、第 8 步 clean 交接、第 9 步唯一工程架构和严格第 10 步交接 | 唯一 `docs/backlog/backlog.md` 已 tracked、提交锚点有效、全仓 `main` / clean；success 进入 `phase_1:initialize_requirement_registry` / step 12 |
| 12 解析 Backlog 并初始化需求注册表 | Responses/Pydantic 静态提取与 Python 三方 Markdown/模型校验；不调用 Claude Agent、`AgentDecision` 或 Skill | 第 11 步完整 success、唯一已 tracked Backlog、root `main` SHA 和初始化状态 | result 保存来源与静态 catalog，state 初始化全 pending 注册表，推进 `phase_1:select_requirement` / step 13；零产品或 Git 写操作 |
...
自动化当前基线为第 12 步 17 项、第 13 步 28 项、第 14 步 20 项，第 12～14 步定向 65 项、全量 243 项；`compileall`、`git diff --check`、IDE 通过，独立审查发现的 4 项中置信问题均已修复并补测试，最终复核无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
独立核验 root/frontend/backend 均为自身 top-level、`main`、clean，文档 tracked。同 run 幂等重跑后 conversation 仍 11 条，session 和 root HEAD 不变，无 Agent、decision 或新提交调用。第 9 步真实事实保持不变；第 10 步随后已按当前合同完成并推进第 11 步。

#### 第 10 步已实现合同与真实验证

当前 Demo v1 的适用性唯一取 `steps/08.json` 与 state 完全一致的 `applicable_repositories` 是否含 `frontend`，不扫描目录，也不让 Agent 判断。这是当前前后端交付单元模型的 Demo 简化；通用 `ui-ux-framework` Skill 保持可处理更广既有项目与显式既有路径的语义，不能把当前固定路径泛化为永久限制。

无 `frontend` 时，步骤仅读第 8 步交接和严格第 9 步 success（唯一 `docs/design/工程架构设计.md`）。无论 state 为 fresh、failed、blocked 或已推进，只要存在本步骤 session、conversation、私有执行状态或历史文件即拒绝；否则零 Git、Agent、负责人决策、LLM 配置和 `docs/ui-ux/` 副作用，写入 success、`applicable: false`、`outputs: []` 并推进 `project:11_requirement_breakdown`。此路径仅有自动化覆盖，黄金项目不走此分支。

有 `frontend` 时，输入严格为第 2 步两份 outputs、第 5 步清单、第 7 步技术方案、第 9 步唯一工程架构文档、第 8 步权威仓库及实际 `frontend`。单一 key/session 为 `ui_ux_framework`，初始 prompt 首行 `/ui-ux-framework`，明确 `bootstrap`，只允许创建或更新 `docs/ui-ux/framework.md`。文档须区分工程事实、已确认决定、目标、假设和待确认事项；禁止单需求设计、代码、配置、项目规则与 Git 写操作。

`completed` verifier 先 repair 缺失或空文档，之后无论有无 diff 都在原 session exact 发送一次 `/commit-changes`。提交锚点要求 exact prompt、紧邻非空 Agent `user` 回复和原 session。最终文档必须非空、普通、非符号链接且 tracked；所有权威仓库均为自身 top-level、`main`、clean；Python 只读 Git。fresh/resume/blocked/idempotency/symlink/success 写入中断与第 9 步同构，局部实现，不改 `common`，不抽 commit DSL。`run_step.py` 对完整第 8～12 步 success 保护既有 result 和已推进 state；第 10 步只接受 true 加固定唯一 output 或 false 加空 outputs，残缺 success 不保护。

自动化已通过第 10 步本体 16 项与 CLI 5 项；当前第 12 步 17 项、第 13 步 28 项、第 14 步 20 项，第 12～14 步定向 65 项、全量 243 项通过。独立审查曾发现无 frontend 的 failed、blocked 或已推进状态遗留执行产物会误报 skip success，现已修正为所有状态统一拒绝并补测试。

真实 run `pcm-demo/runs/step01-mendmark` 与产品 `/Users/zhou/resource/fireworks/ANDRSZAN/pcm-products/mendmark` 的第 8 步权威仓库为 `root/frontend/backend`，故适用。session `2d052d9a-b9fd-4fcd-b5f9-ff724f25dbc1` 的 init 确认 skill/slash command 已加载，cwd 为产品根，模型 `claude-fable-5[1M]`，Claude Code 2.1.233，permissionMode `bypassPermissions`。最后 Agent success，5 turns，cost `$3.207112`，`terminal_reason=completed`，无错误；fresh 运行内置 Explore 子代理曾输出未识别模型 `gpt-5.6-terra[1m]` 警告，主 Agent 同一次调用继续并 success，未追加人工或 run-history 恢复提示。
...
固定输入严格为第 2 步两份 outputs、第 5 步清单、第 7 步技术方案、第 8 步 result/state 一致的权威仓库 clean 交接、第 9 步唯一 `docs/design/工程架构设计.md`，以及严格第 10 步 success。第 10 步 `applicable: true` 时读取唯一 `docs/ui-ux/framework.md`；`applicable: false` 时必须为 `outputs: []`，不读取或扫描该文档。程序不从默认路径、目录扫描、历史文字或 Agent 回复猜测交接。单一 key/session 为 `requirement_breakdown`，初始 prompt 首行 `/requirement-breakdown`，单次上限 48 turns、`$16`，同一历史最多 8 轮决定；唯一固定输出为 `docs/backlog/backlog.md`。

Agent 只能创建或更新该 Backlog，不得实施需求或修改代码、测试、配置、项目规则、其它文档，初始工作不得 Git 写操作。Backlog 形成可独立交付、可验证、顺序合理的正式需求，包含范围、目标、验收要点、依赖和风险；它不记录待开发、开发中、已完成、阻塞等需求开发状态。当前活动需求、完成情况和恢复位置由调用方外部结构化状态管理；业务对象或业务流程自身的状态仍可写入需求内容。

运行时全部权威仓库必须是自身 top-level、`main`；子仓一直 clean，root 只允许固定 Backlog dirty，Python 仅执行 Git 只读核验。负责人 `completed` 后先 repair 缺失或空文档，随后无论有无 diff 都在原 session exact 发送一次 `/commit-changes`。exact prompt、紧邻非空 Agent `user` 回复和原 session 构成提交锚点；最终文件为非空、非符号链接、tracked 普通文件，且全仓 clean。fresh 拒绝既有执行产物，resume 必须具备原 session、conversation 引用和非符号链接历史；blocked 只对应不可替代外部资源，failed 保留现场，完整 success 可从 result/state 写入中断恢复并幂等复验。第 8～11 步的 Git/提交合同都保持局部实现，不改 `common`，不抽 Git DSL。

成功 `steps/11.json` 严格为 `applicable: true` 和唯一 output；第 11 步自身的 success 交接进入 `phase_1_requirement_development` / `phase_1:initialize_requirement_registry`，`step/current_step` 为 12，`active_requirement`、`requirement_cycle` 为 `null`，且不写需求注册表。当前 `run_step.py` 已支持第 0～14 步：完整第 8～12 步 fixed success 与当前 active requirement 的第 13、14 步 scoped success 分别受保护。
...
自动化通过第 12 步 17 项、第 13 步 28 项、第 14 步 20 项，第 12～14 步定向 65 项、全量 243 项；`compileall`、`git diff --check` 通过，IDE 对第 12、13 步与 `run_step.py` 无诊断，独立审查发现的 4 项中置信问题均已修复并补测试，最终复核无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
### 阶段一：已实现第 12～14 步；第 15～18 步循环待实现

第 12 步位于需求循环之外；第 13 步已选择首个需求并建立统一分支，第 14 步已形成 BR-001 活动 TRD。`step01-mendmark` 当前位于 `requirement:15_development` / step 15；第 15～18 步仍是当前已知正式 Backlog 的未来循环。

#### 第 12 步已实现合同与真实运行

- **执行方式**：直接调用 OpenAI Python SDK `responses.parse`，以 Pydantic 输入输出；不调用 Claude Agent SDK、`AgentDecision` 循环或 Skill。
- **权威输入与静态模型**：只接受第 11 步完整 success、产品根已 tracked 的唯一 `docs/backlog/backlog.md`、根仓当前 `main` SHA 和当前运行状态。模型每项仅允许 `id`、`title`、`order`、`depends_on`；system prompt 只描述 Backlog 静态提取、输出和候选约束，不包含步骤编号、PCM、session 或 Skill 编排背景。
- **三方解析与校验**：Python 要求唯一正式大需求总览表、唯一需求详情区、详情卡 ID/标题/顺序、总览依赖及每卡唯一 `##### 前置依赖` 一致；再与模型输出逐项严格比对。ID 与依赖 ID 仅接受 `[A-Za-z0-9][A-Za-z0-9_-]*`，ID 忽略大小写后唯一；拒绝遗漏、虚构、非连续 order、标题/依赖不一致、依赖不存在、重复、自依赖或环。
- **现场与副作用边界**：root 必须是自身 Git top-level、`main`、clean，有有效 `HEAD`，Backlog 为非空、非符号链接普通文件且 tracked；不选择活动需求、不创建分支、不修改产品项目、不执行 Git 写操作或提交。
- **结果、状态与恢复**：成功 result 保存 Backlog `{path, sha256, root_main_sha}` 来源与仅静态字段的 catalog，`outputs: []`；先写 result，后写 `schema_version: 1` 注册表，全部需求由 Python 初始化 `pending`、`completion: null`，然后进入 `phase_1:select_requirement` / step 13。JSON 以同目录唯一临时文件原子 replace 写入。result 已写而 state 未推进时，从 result 恢复；完整 success 重跑要求来源、catalog、注册表完全一致并零模型调用，任一漂移或冲突 `failed` 且不覆盖。所有输入、状态、文件、Git、模型或校验错误均为 `failed`。
- **真实运行**：`step01-mendmark` 先严格核验旧 state，再仅 run-local 规范化旧第 11 步入口，随后真实 Responses 成功注册 BR-001～BR-014 共 14 项；标题、order 1～14 与依赖均同 Backlog 总览和详情卡一致，全部 `pending`、`completion: null`。Backlog SHA-256 为 `707c4b91924542e9cbd282fba53cc8857b8ea1fcbdfb7a820c208d0573d759bb`，root `main` SHA 为 `0232d8136c075cb61a6617a95e1504b67bd9acd1`。state 到第 13 步，活动需求和 cycle 为空，无新 Claude session 或负责人决策 conversation，未新增 `completed_requirements` 或 `phase_two`。产品 root/frontend/backend 分别为 `0232d8136c075cb61a6617a95e1504b67bd9acd1`、`dbab574dbe4d83a02323a750afd04de007565ac5`、`9682be837759c20f1a9ebbdf8fa2cfc09c2768d4`，均为 `main`、clean。
- **幂等真实重跑**：以不可用模型配置运行 CLI 仍 success，证明未加载模型；`steps/12.json` 与 state 字节不变，最终 result SHA-256 为 `8f829cb3d4935a9dcd07bea2dd8f0df2a22369c433ba4320938e2a9461f41fb0`，state SHA-256 为 `73fd0cb99c39f64f9ef210a171799f79baaee87a417a18f5db74f5b5374470f7`。
...
| 14 | 形成活动 TRD（已实现） | fresh 全局预检 requirement branch/base/clean 后先持久化 `docs/trd/<YYYY-MM-DD>-<ID>-<标题>.md`，再使用 requirement-scoped session 显式调用 `/trd-design`。默认 Agent 无 Bash，Write/Edit 只允许 exact TRD；success 写 `steps/requirements/<ID>/14.json` 并推进 `requirement:15_development`，TRD 保留未提交 |
| 15 | 实现与验证 | 调用 `dev-workflow` 完成实现、测试、构建、运行、联调、真实浏览器与渲染验收和独立审查；稳定设计偏差同步活动 TRD；保存 `development_session_id`，不提交、不合并 |
| 16 | 原开发 session 规则复盘 | 恢复第 15 步同一 `development_session_id` 调用 `session-rule-retrospective`，不得新建替代 session；只允许本次调用增量修改 root `.claude/rules/`，比较调用前后增量而非全量 dirty 范围；不提交 |
...
第 14 步 strict 消费 current active requirement、注册表唯一 active 项、cycle、第 13 步 scoped success，以及第 2/5/7/9/11 步固定文档和第 10 步条件 UI/UX 输入。fresh 在任何 state/Agent 副作用前核验全部适用仓路径、top-level、统一需求分支、`HEAD/main/target/base`、index/worktree 和 merge/rebase/cherry-pick/revert/bisect 状态。Python 使用首次本地日期、ID 和标题生成 exact `trd_path`；路径、标题、长度、符号链接和父路径冲突通过后先写 cycle intent，再启动 Agent。

Agent/session/conversation key 为 `trd_design_<ID>`。公共 Claude runner 增加默认不影响其它步骤的可选 `tools/hooks` 透传；第 14 步默认仅暴露 `Read/Glob/Grep/Write/Edit/Skill`，没有 Bash、Agent 或 NotebookEdit，PreToolUse hook 只允许 exact TRD。负责人规则要求阻碍实现的高影响决定已经采用当前基线，不能把“实现前必须确认”列表误作完成。Python completion verifier 只核验文档非空、root 只有 exact 未跟踪 TRD、其它仓 clean、index clean、refs 等于 base 和 session/conversation 完整；不解析 Markdown，也不调用 commit。

真实 run 中，用户授权的 Skill 命名提交先导致旧 root base 漂移；按用户选择归档 run、确认旧需求分支无独有提交、安全删除并重跑第 13 步后，root base 更新为 `a7d5509df6843a06315aa803d87285569b86e355`。第 14 步使用 session `b9ed4756-0acf-4666-b3f9-c8f3628c03f1` 生成 `docs/trd/2026-08-25-BR-001-身份、角色访问与站内消息入口.md`。首次负责人错误接受 8 项实现门槛，主代理读取实际文档后拒绝语义完成，收紧生产规则并追加一次普通复核指令，原 session 收敛决定。负责人服务随后因 free quota 403 保持尾部 Agent 回复；额度恢复后原节点裁决 completed。最终 conversation 7 条，无 `/commit-changes`；state 位于 step 15，root 仅唯一 TRD 未跟踪，frontend/backend clean，无 staged、提交、merge 或 push。幂等重跑 state/result/conversation/TRD 字节不变。
...
第 0～14 步同时使用 `current_step` 和必要的 `phase/current_node`。第 9～11 步 success 均以各自固定 commit prompt 紧邻非空 Agent 回复和原 session 作为执行锚点，再以固定文档 tracked、全仓 clean 兜底；第 10 步不适用 success 则必须为 `applicable: false`、空 outputs 且没有执行产物。`run_step.py` 对 schema 完整的第 8～12 步 success 保持固定保护；第 13、14 步分别只对当前 active/cycle 的完整 scoped success 保护 result/state，残缺、blocked 或 failed result 可恢复覆盖。

当前真实 state 位于 `phase_1_requirement_development` / `requirement:15_development`、`step/current_step: 15`。注册表含 14 项 BR：`BR-001` 为 active、13 项 pending、0 项 completed；`active_requirement` 仅为 `BR-001`，cycle 保存 `req/br-001`、各仓 `base_sha`、exact `trd_path` 和返回节点。第 14 步完整 scoped success 指向 session `b9ed4756-0acf-4666-b3f9-c8f3628c03f1` 和唯一活动 TRD；推进后幂等重跑不读取 Git、不调用 Agent 或负责人服务，state/result/conversation/TRD 字节不变。root 仅该 TRD 未跟踪，frontend/backend clean，三仓 index 为空且 refs 等于 base。第 15 步及以后仍未实现。
...
第 12 步 success 后、第 13 步执行前的真实中间恢复锚点为：
...
```json
{
  ...
  "requirement_registry": {
    ...
    "source": {
      ...
      "sha256": "707c4b91924542e9cbd282fba53cc8857b8ea1fcbdfb7a820c208d0573d759bb",
      "root_main_sha": "0232d8136c075cb61a6617a95e1504b67bd9acd1"
    },
    "requirements": [
      {
        "id": "BR-001",
...
实际注册表包含 BR-001～BR-014 共 14 项；上述只展示首项结构。完整 success 的 result 先于 state 写入，result 已写而 state 未推进时可无模型调用恢复；完成后完整 success、当前 Backlog/根仓来源、静态 catalog 和全 pending 注册表必须严格一致，否则失败且不覆盖。
...
9. 第 10 步先以第 8 步 result/state 一致的 `applicable_repositories` 判断是否有 `frontend`。不适用时拒绝所有状态遗留执行产物，严格第 9 步 success 后以 `applicable: false`、空 outputs 推进第 11 步且零调用；适用 fresh 入口要求全仓 clean、无预置执行产物，恢复必须具有原 session、conversation 引用和非符号链接历史。固定文档缺失/空先 repair，随后原 session exact `/commit-changes`，以固定 prompt 后紧邻非空 Agent 回复、文档 tracked 和全仓 clean 作为锚点；适用与不适用写入中断均可按完整 success 结果恢复，残缺 success 不保护；
10. 第 11 步 fresh 入口要求全仓 clean 且没有预置执行产物；恢复必须具备原 session、conversation 引用和非符号链接历史。缺失或空 Backlog 先 repair，随后在原 session exact `/commit-changes`；固定 prompt 后紧邻非空 Agent 回复、文件 tracked 与全仓 clean 构成锚点。完整 success 可从 result/state 中断恢复，残缺 success 不保护；
...
第 3～8 步的历史和真实验证事实保持不变。第 9 步旧失败已定位为公共 prompt 结构问题并修复；fresh 运行使用 session `f41fc439-4c46-434f-b3e9-d15c18c89601` 完成 11 条 conversation、固定文档提交、合法最终 decision、状态推进、独立 Git 核验和幂等重跑。第 10 步随后使用 session `2d052d9a-b9fd-4fcd-b5f9-ff724f25dbc1` 完成固定 UI/UX 文档、提交、独立核验和幂等重跑。第 11 步以 session `af7d198c-b090-44d8-9d92-ae0738150854` 完成固定 Backlog、同 session exact 提交、独立 Git 核验和幂等重跑；历史旧第 12 步占位 state 经严格 run-local 规范化后，第 12 步以真实 Responses 完成 14 项注册、状态推进和零模型幂等重跑。第 13 步完成 BR-001 选择和三仓 `req/br-001`；授权 Skill 提交导致 root base 漂移后，按用户选择归档并从最新 main 重跑第 13 步。第 14 步使用 session `b9ed4756-0acf-4666-b3f9-c8f3628c03f1` 形成活动 TRD、完成同 session 语义复核、合法最终 decision、Git 边界核验和幂等重跑；当前后续从第 15 步开始。
...
- 第 10 步本体 16 项与 CLI 5 项：第 8 步 result/state 一致的确定性适用性、无 frontend 的零调用与全状态执行产物拒绝、严格第 9 步交接、适用输入/prompt/单一 session、固定输出、文档 repair、exact commit prompt 与相邻回复锚点、worktree 边界、Git 只读 allowlist、fresh/blocked/resume、符号链接、写入中断、幂等与 CLI success 保护；
- 第 11 步本体 9 项与 CLI 6 项：严格第 2/5/7/8/9/10 交接、第 10 步 true/false UI/UX 读取边界、单一 session/固定 prompt/预算与决定上限、无需求开发状态 Backlog 边界、文档 repair、exact commit 与相邻回复锚点、工作树与 Git 只读 allowlist、fresh/blocked/resume、符号链接、写入中断、幂等、迁移后的初始化入口和 CLI success 保护；
- 第 12 步本体 11 项与 CLI 6 项：Pydantic 模型字段限制、合法 ID/依赖 ID 语法与忽略大小写唯一性、唯一总览/详情/前置依赖三方比对、模型遗漏/虚构和静态合同拒绝、root top-level/main/clean/HEAD/tracked 核验、零产品与 Git 写操作、旧入口拒绝、result→state 中断恢复、来源/catalog/注册表漂移拒绝、失败重跑、严格 success 保护和失败脱敏；
...
- 第 12～14 步定向共 65 项；公共 Agent 决策循环与 Claude runner、第 13、14 步定向共 73 项；从 `pcm-demo/` 根递归发现 `common/` 与 `steps/` 的全量 243 项 `unittest` 已通过；
- Probe C 和旧四段 XML 回放只作为历史；现行五段 prompt 已由第 9、10、11 步 fresh 真实运行验证；
...
- 第 10 步真实 run 已覆盖适用 fresh 入口、固定 UI/UX 文档生成、一次 exact commit prompt、合法两次 completed、固定文档提交、状态推进、独立核验和幂等重跑；不适用路径仅由自动化覆盖；
- 第 11 步真实 run 已覆盖 fresh clean 入口、适用的 UI/UX true 交接、固定 Backlog 生成、无需求开发状态边界、一次 exact commit prompt、同 session 提交、独立核验和幂等重跑；历史 success 当时进入旧第 12 步占位入口。第 10 步 false 的空 outputs 和不读取/扫描 UI/UX 文档边界由自动化覆盖。瞬时 Responses failed 由正常原节点重跑恢复，未注入 conversation 的只读诊断不构成正式决定；
...
- 当前验证记录：`compileall`、全量 243 项 `unittest`、第 12～14 步定向 65 项、公共循环与第 13/14 步定向 73 项、`git diff --check` 均已通过；全工作区 IDE diagnostics 无问题，独立审查发现的 4 项中置信问题均已修复并补测试，最终复核无高、中置信发现。Ruff 未安装，未执行 Ruff。

### 2. 后续增量风险

进入对应步骤后再增加最小真实验证：

- 第 14～18 步及后续需求循环复用：第 14～16 步无提交、活动 TRD 同步、`development_session_id` 保存与同 session 复盘增量边界、第 17 步逐 dirty 仓提交和 clean 仓无空提交、第 18 步代码仓先于 root 的 ff-only 合并、拒绝非 fast-forward、部分合并恢复、分支清理和 completed 时序；
...
3. `run_all.py` 在何时建立，以及第 0～14 步现有入口如何与新节点协议复用；
4. 第 12、13 步的 Pydantic/ID、结果、注册表、分支、节点和 failpoint 已按现行合同实现；第 14～18 步开始实现前仍需逐步收敛调用预算、各步新增字段和 failpoint；
...
这些未决事项不改变第 0～14 步已经完成的真实事实，也不重新打开本轮已实现的第 12～14 步合同或已确认的第 15～18 步总体合同。后续从第 15 步开始；每个新步骤继续对照原手稿、当前流程设计和活动 TRD 确认实现级输入、输出、失败、阻塞和恢复细节，随后先实现代码并通过测试和真实验证，再将实际实现同步到设计文档并提交。
