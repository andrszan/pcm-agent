...
>
> 重构前，技术探针以及业务第 0～7 步已真实运行。第 8～15 步已按当前合同完成真实验证；第 14 步预持久化 exact TRD 路径，在 requirement-scoped `trd-design` session 中形成 BR-001 活动 TRD；第 15 步在 `development_<ID>` session 中完成实现、真实验证和独立审查并推进 `requirement:16_rule_retrospective` / step 16。第 14 步前用户授权的 Skill 命名提交使 root 旧 cycle base 漂移；经用户明确选择后归档旧 run、确认旧需求分支无独有提交并从最新 `main` 重跑第 13 步。第 14 步真实运行又发现负责人把实现门槛误判为 completed，已通过同 session 语义复核收敛并收紧生产 completion 规则。第 16 步及以后仍未实现。旧 403、非 JSON、非法 completed 和旧 prompt 设计只保留为已修复的根因历史；`step08-real-20260823-a/b` 继续仅为旧第 8 步合同历史。
...
4. **技术阶段 3：循环前第 12 步与阶段一第 13～18 步**——第 12～15 步代码、自动化和真实验证已完成；下一步从第 16 步逐步跑通复盘、提交和合并；
...
- `run_step.py` 的第 0～15 步单步入口；第 8～12 步保留固定 result success 保护，第 13～15 步分别以 current `active_requirement`/cycle 定位并保护唯一完整 scoped success；
- 第 0 步完整产品初稿无副作用跳过；
...
- 第 16～18 步的代码、测试和真实运行；
- `run_all.py` 完整串联入口；
- 第 16～18 步所需的复盘、提交与合并持久化。

第 0～15 步均已完成代码、自动化和适用真实验证；未实现能力必须返回明确程序错误，不得以空脚本、固定 JSON、旧实现或口头结论冒充成功。
...
- 第 15 步本体 6 项与 CLI 4 项，共 10 项及全量 252 项测试均已通过；`compileall`、`git diff --check` 通过，IDE 无新增诊断，独立只读审查无高、中置信发现；Ruff 未安装，未执行 Ruff；
...
- 当前验证记录：第 15 步本体 6 项与 CLI 4 项、全量 252 项 `unittest`、`compileall`、`git diff --check` 均通过；全工作区 IDE diagnostics 无新增问题，独立只读审查无高、中置信发现；第 8～15 步真实验收完成。本轮文档同步不替代第 16～18 步验证。
...
- `run_step.py`：当前运行已经实现的第 0～15 步；实际命令和配置说明以 `pcm-demo/README.md` 及各步骤 README 为准；
...
自动化当前基线为第 15 步本体 6 项与 CLI 4 项，共 10 项、全量 252 项；`compileall`、`git diff --check`、IDE 通过，独立只读审查无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
自动化已通过第 10 步本体 16 项与 CLI 5 项；当前第 15 步本体 6 项与 CLI 4 项、全量 252 项通过。独立审查曾发现无 frontend 的 failed、blocked 或已推进状态遗留执行产物会误报 skip success，现已修正为所有状态统一拒绝并补测试。
...
成功 `steps/11.json` 严格为 `applicable: true` 和唯一 output；第 11 步自身的 success 交接进入 `phase_1_requirement_development` / `phase_1:initialize_requirement_registry`，`step/current_step` 为 12，`active_requirement`、`requirement_cycle` 为 `null`，且不写需求注册表。当前 `run_step.py` 已支持第 0～15 步：完整第 8～12 步 fixed success 与当前 active requirement 的第 13～15 步 scoped success 分别受保护。
...
当前自动化通过第 15 步本体 6 项与 CLI 4 项、全量 252 项；`compileall`、`git diff --check` 通过，IDE 无新增诊断，独立只读审查无高、中置信发现。Ruff 未安装，未执行 Ruff。
...
### 阶段一：已实现第 12～15 步；第 16～18 步循环待实现

第 12 步位于需求循环之外；第 13 步已选择首个需求并建立统一分支，第 14 步已形成 BR-001 活动 TRD，第 15 步已完成实现和验证。`step01-mendmark` 当前位于 `requirement:16_rule_retrospective` / step 16；第 16～18 步仍是当前已知正式 Backlog 的未来循环。
...
Agent/session/conversation key 为 `trd_design_<ID>`。第 14 步直接复用公共 `run_claude()`，不扩展其参数，也不在领域步骤覆盖 `permission_mode`、`tools`、`allowed_tools`、`disallowed_tools` 或增加私有 tool hook；项目 `.claude/settings.json` 及 Claude Code 默认设置加载语义继续是统一权威来源。负责人规则要求阻碍实现的高影响决定已经采用当前基线，不能把“实现前必须确认”列表误作完成。Python completion verifier 只核验文档非空、root 完成现场只有 exact 未跟踪 TRD、其它仓 clean、index clean、refs 等于 base 和 session/conversation 完整；不解析 Markdown，也不调用 commit。该核验确认最终交接不存在 Git 写入结果或越界变更，不声称对整个 Agent 执行历史作绝对取证；若未来需要更强隔离，应建立项目级权限或沙箱合同。

真实 run 中，用户授权的 Skill 命名提交先导致旧 root base 漂移；按用户选择归档 run、确认旧需求分支无独有提交、安全删除并重跑第 13 步后，root base 更新为 `a7d5509df6843a06315aa803d87285569b86e355`。第 14 步使用 session `b9ed4756-0acf-4666-b3f9-c8f3628c03f1` 生成 `docs/trd/2026-08-25-BR-001-身份、角色访问与站内消息入口.md`。首次负责人错误接受 8 项实现门槛，主代理读取实际文档后拒绝语义完成，收紧生产规则并追加一次普通复核指令，原 session 收敛决定。负责人服务随后因 free quota 403 保持尾部 Agent 回复；额度恢复后原节点裁决 completed。最终 conversation 7 条，无 `/commit-changes`；state 位于 step 15，root 仅唯一 TRD 未跟踪，frontend/backend clean，无 staged、提交、merge 或 push。幂等重跑 state/result/conversation/TRD 字节不变。步骤私有工具覆盖在该真实 Agent 运行后经用户复核删除；没有为此重跑或改写成功 session，当前生产路径使用公共默认 runner，自动化确认不传 `tools/hooks`，既有产物和 Git 现场不变。

#### 第 15 步已实现合同与真实验证

第 15 步只消费当前 state 的 active requirement/cycle/workspace、当前需求 scoped 第 14 步 `success` 和非空活动 TRD；不读取第 2/5/7/8/9/10/11/13 步文档，不执行 Git 命令或 Git verifier。requirement-scoped key 为 `development_<ID>`，复用公共 `run_agent_decision_loop`。prompt 只包含 `/dev-workflow`、需求 ID/标题和活动 TRD 路径；Agent 按需自行读取项目资料、代码、配置、测试和环境，允许把稳定设计偏差同步活动 TRD。它不得修改 `.claude/rules/`，也不得 stage、commit、创建或切换分支、merge 或 push。

completion verifier 为空，负责人 `AgentDecision.completed` 即领域完成；`continue` 和 `blocked` 沿用公共语义。首次从 Agent 取得 session 后立即同步 `requirement_cycle.development_session_id`。success scoped result 写入 `steps/requirements/<ID>/15.json`，`outputs: []`，保存 requirement ID、TRD 路径和 development session ID，随后推进 `requirement:16_rule_retrospective` / step 16；blocked 保留 step 15 和同一 session。success result→state 中断恢复及推进后幂等已实现，CLI 只保护当前活动需求完整 scoped success。
...
第 0～15 步同时使用 `current_step` 和必要的 `phase/current_node`。第 9～11 步 success 均以各自固定 commit prompt 紧邻非空 Agent 回复和原 session 作为执行锚点，再以固定文档 tracked、全仓 clean 兜底；第 10 步不适用 success 则必须为 `applicable: false`、空 outputs 且没有执行产物。`run_step.py` 对 schema 完整的第 8～12 步 success 保持固定保护；第 13～15 步分别只对当前 active/cycle 的完整 scoped success 保护 result/state，残缺、blocked 或 failed result 可恢复覆盖。

当前真实 state 位于 `phase_1_requirement_development` / `requirement:16_rule_retrospective`、`step/current_step: 16`。注册表含 14 项 BR：`BR-001` 为 active、13 项 pending、0 项 completed；`active_requirement` 仅为 `BR-001`，cycle 保存 `req/br-001`、各仓 `base_sha`、exact `trd_path`、`development_session_id: e6bd1b82-39f9-41b2-9cc9-a69b281015dc` 和返回节点。第 15 步完整 scoped success 指向同一 development session；不可用 LLM 配置的幂等重跑不读取 Agent 或负责人服务，state/result/conversation 字节不变。root 保留活动 TRD和代表性截图未跟踪，frontend/backend保留实现、测试、迁移与配置未提交变更；三仓 index 为空且仍在 `req/br-001`。第 16～18 步仍未实现。
...
第 3～8 步的历史和真实验证事实保持不变。第 9 步旧失败已定位为公共 prompt 结构问题并修复；fresh 运行使用 session `f41fc439-4c46-434f-b3e9-d15c18c89601` 完成 11 条 conversation、固定文档提交、合法最终 decision、状态推进、独立 Git 核验和幂等重跑。第 10 步随后使用 session `2d052d9a-b9fd-4fcd-b5f9-ff724f25dbc1` 完成固定 UI/UX 文档、提交、独立核验和幂等重跑。第 11 步以 session `af7d198c-b090-44d8-9d92-ae0738150854` 完成固定 Backlog、同 session exact 提交、独立 Git 核验和幂等重跑；历史旧第 12 步占位 state 经严格 run-local 规范化后，第 12 步以真实 Responses 完成 14 项注册、状态推进和零模型幂等重跑。第 13 步完成 BR-001 选择和三仓 `req/br-001`；授权 Skill 提交导致 root base 漂移后，按用户选择归档并从最新 main 重跑第 13 步。第 14 步使用 session `b9ed4756-0acf-4666-b3f9-c8f3628c03f1` 形成活动 TRD、完成同 session 语义复核、合法最终 decision、Git 边界核验和幂等重跑；第 15 步使用 session `e6bd1b82-39f9-41b2-9cc9-a69b281015dc` 完成实现、真实验证、浏览器矩阵和幂等重跑；当前后续从第 16 步开始。
...
- 第 15 步本体 6 项与 CLI 4 项，共 10 项；从 `pcm-demo/` 根递归发现 `common/` 与 `steps/` 的全量 252 项 `unittest` 已通过；
...
- 当前验证记录：`compileall`、全量 252 项 `unittest`、第 15 步本体 6 项与 CLI 4 项、`git diff --check` 均已通过；全工作区 IDE diagnostics 无新增问题，独立只读审查无高、中置信发现。Ruff 未安装，未执行 Ruff。

### 2. 后续增量风险

进入对应步骤后再增加最小真实验证：

- 第 16～18 步及后续需求循环复用：第 16 步同 session 复盘增量边界、第 17 步逐 dirty 仓提交和 clean 仓无空提交、第 18 步代码仓先于 root 的 ff-only 合并、拒绝非 fast-forward、部分合并恢复、分支清理和 completed 时序；
...
3. `run_all.py` 在何时建立，以及第 0～15 步现有入口如何与新节点协议复用；
4. 第 12、13 步的 Pydantic/ID、结果、注册表、分支、节点和 failpoint 已按现行合同实现；第 14、15 步已按各自合同实现，第 16～18 步开始实现前仍需逐步收敛调用预算、各步新增字段和 failpoint；
...
这些未决事项不改变第 0～15 步已经完成的真实事实，也不重新打开本轮已实现的第 12～15 步合同或已确认的第 16～18 步总体合同。后续从第 16 步开始；每个新步骤继续对照原手稿、当前流程设计和活动 TRD 确认实现级输入、输出、失败、阻塞和恢复细节，随后先实现代码并通过测试和真实验证，再将实际实现同步到设计文档并提交。
